...
@copyright (c) 2014      Damien Regad, Mark Newnham and the ADOdb community
  ...
  as of ADOdb version 5.20.0. Use the mysqli_ driver instead, which supports both
...
class ADODB_mysql extends ADOConnection {
	...
	function SetCharSet($charset_name)
	{
		...
		if ($this->charSet !== $charset_name) {
			$ok = @mysqli_set_charset($charset_name,$this->_connectionID);
			if ($ok) {
	...
	function BeginTrans()
	{
		if ($this->debug) ADOConnection::outp("Transactions not supported in 'mysql' driver. Use 'mysqlt' or 'mysqli_' driver");
	}
...
//		return "from_unixtime(unix_timestamp($date)+$fraction)";
	...
	global $ADODB_COUNTRECS;
		if($ADODB_COUNTRECS)
			...
			return @mysqli_unbuffered_query($sql,$this->_connectionID); // requires PHP >= 4.0.6
	*/
	}
