...
//                        }
                        ...
                        $db->Execute($sql2);
						 ...
						 if ($cardFee != ''){
                            $bill_obj->updateCardFeeBill($encounter_nr,$pid);
                         }
                        
						 if ($bloodSugarFee != ''){
                            $bill_obj->updateBloodFeeBill($encounter_nr,$pid);
                         }
...
//    $smarty->assign('LDRecBy', $LDReg);
    $smarty->assign('LDRecBy', "Card FEE");
    //CARD FEE
    $rTemp = $rTemp."Charge Card Fee:<input type='checkbox' name='cardFee' id='cardFee'> &nbsp;&nbsp;";

	$smarty->assign('LDRecBy', "Blood Sugar");
    //CARD FEE
    $rTemp = $rTemp."Charge Blood Sugar:<input type='checkbox' name='bloodSugarFee' id='bloodSugarFee'>";
...
//    }
    ...
    $nTemp = $nTemp . '<select name="referals">
			<option value=""></option>';
        $nTemp = $nTemp . '<option value="Referal from Other Health Facility">Referal from Other Health Facility</option>';
        $nTemp = $nTemp . '<option value="Referal from Community Unit">Referal from Community Unit</option>';

    $nTemp = $nTemp . '</select>';

    $smarty->assign('LDReferals', $LDReferals);
    $smarty->assign('referals', $nTemp);
...
//    }
    ...
    $sTemp = '<input type="hidden" name="pid" value="' . $pid . '">
				...
				<input type="hidden" name="myform_key" value="'. md5(date('Y:m:d H:i:s')) .'">';
