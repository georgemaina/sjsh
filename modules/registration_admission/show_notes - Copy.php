...
if(isset($_REQUEST['pn'])){
        $encounter_nr=$_REQUEST['pn'];
        $_SESSION['sess_en']=$_REQUEST['pn'];
}else{
	$encounter_nr=$_SESSION['sess_en'];
}

 $consultation=$bill_obj->checkConsultationPayment($encounter_nr);
if($consultation){
        if(!isset($mode)){
                $mode='show';
    ...
    header("location:../validateFile.php?target=Notes&allow_update=1&pid=".$_SESSION['sess_pid']);
    exit;
}
?>
