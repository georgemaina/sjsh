...
$pid=$_REQUEST[pid];
$validateSource=$_REQUEST[validateSource];
...
switch ($task) {
    ...
    case "validatePatient":
        validatePatient($pid,$validateSource);
        break;
    case "getEncounterVitals":
        getEncounterVitals($encounterNo);
        break;
    case "showVitals";
        showVitals($encounterNo);
        break;  
    //default:
      //  echo "{failure:true}";
      //  break;
...
function showVitals($encounterNo){
        $results=getMeasurements($encounterNo);
...
//                     break;
      ...
      echo $Systolic .",". $Diastolic .",". $Weight .",". $Height .",". $Bmi;
}


function getEncounterVitals($encounterNo){
   global $db;
   $debug=false;
   
   $sql="SELECT  msr_type_nr,`value` FROM care_encounter_measurement where encounter_nr=$encounterNo";
   if($debug) echo $sql;
   ...
   echo '{"vitals":[';
       ...
       while ($row = $results->FetchRow()) {
           echo '{"Vital":"'. $row[msr_type_nr].'","Value":"'. $row[value].'"}';
...
function getTreatmentRegister($strDate1,$strDate2){
    ...
    if($strDate1<>'' and $strDate2<>''){
        $sql.=" where ScreeningDate between '$strDate1' and '$strDate2'";
    }
...
//                    break;
        ...
        echo '{"PID":"'. $row[0].'","PatientName":"'. $row[1].' '. $row[2].' '. $row[3].'","ScreeningDate":"'. $row[encounter_date]
            ...
            .'","County":"MERU","Location":"'. $row[citizenship].'","hhaUpdate":"'. $row[hhaUpdate]
...
function validatePatient($pid,$validateSource){
    global $db;
    $debug=false;
...
function validateEncounter($encounterNr){
    global $db;
    $debug=false;
    $psql = "SELECT DISTINCT encounterNR FROM care_hha_vitals WHERE encounterNr=$encounterNr";
    if($debug) echo $psql;
...
function getPatientDetails($encounterNr){
    global $db;
    $debug=false;
    
    $sql="SELECT p.pid,MAX(e.`encounter_nr`) as encounterNr,CONCAT(name_first,' ',name_2,' ',name_last) AS pname,p.`nat_id_nr`,p.`phone_1_nr`,
                p.`date_birth`,p.`sex`,p.`citizenship`
            FROM care_person p
            LEFT JOIN care_encounter e ON p.`pid`=e.pid
            WHERE e.`encounter_nr`='$encounterNr'
...
function getCompanyDetails(){
    ...
    if($global_result->RecordCount()>0){
        $row=$global_result->FetchRow();
        return $row;
    }else{
        return '0';
    }
...
function saveInitialEncounter($questions) {
    global $db;
    $debug=false;
    $error=0;
    ...
    $encounterDetails=getPatientDetails($questions[encounterNr]);
    ...
    $uniqueID=getUniqueID($encounterDetails[nat_id_nr],$encounterDetails[phone_1_nr]);
    ...
    $sql2 = "INSERT INTO `care_hha_patients`
            ...
            VALUES
              ( '$questions[pid]','$encounterDetails[pname]','$companyDetails[CompanyName]','$companyDetails[mflCode]'
                ,'$ScreeningDate','$uniqueID','$encounterDetails[nat_id_nr]'
                ,'$companyDetails[County]','$encounterDetails[date_birth]','$encounterDetails[sex]','Yes'
                ,'$encounterDetails[phone_1_nr]','$inputUser','$encounterDetails[citizenship]','K1RINYAGA')";

    if($debug) echo $sql2;


    if( validatePatient($questions[pid],0)>0){
        $error=4;
    }else{
        if ($db->Execute($sql2)) {
//            if($_REQUEST[Vitals]>0){
                $error=insertVitals($questions,$inputTime,$inputDate,$inputUser);
//            }

//            if($_REQUEST[Questions]>0) {
                $error=insertQuestions($questions,$inputTime,$inputDate,$inputUser);
//            }
...
function insertVitals($encounterDetails,$inputTime,$inputDate,$inputUser){
    ...
    if($encounterDetails[BPSecondReading1]>130 and $encounterDetails[BPSecondReading1]<=139 and $encounterDetails[BPSecondReading2]>85 and $encounterDetails[BPSecondReading2]<=89){
        $pre_hype='Y';
    }else{
        $pre_hype='N';
    }
    ...
    $sql1="INSERT INTO `care_hha_vitals` (`FormID`,`PID`,`EncounterNr`,`InputDate`,`InputTime`,`BPInitial1`,`BPInitial2`,
                        ...
                        VALUES ('1','$encounterDetails[pid]','$encounterDetails[encounterNr]','$inputDate'
                        ,'$inputTime','$encounterDetails[BPFirstReading1]','$encounterDetails[BPFirstReading2]','$encounterDetails[BPFirstReading1]'
                        ...
                        ,'$encounterDetails[Weight]' ,'$encounterDetails[BMI]','$inputUser',
                        'Clinician','$inputUser','$newPatient','$returnPatient')";
...
function insertContinuationVitals($encounterDetails,$inputTime,$inputDate,$inputUser){
    global $db;
    $debug=false;
    ...
    $sql1="INSERT INTO `care_hha_vitals` (`FormID`,`PID`,`EncounterNr`,`InputDate`,`InputTime`,`BPInitial1`,`BPInitial2`,
                          `BPFirstReading1`,`BPFirstReading2`,`BPSecondReading1`,`BPSecondReading2`,`Normal`,`Pre_hypertensive`,`Hypertensive`,
                          `Height`,`Weight`,`BMI`,`Clinian`,`Designation`,`InputUser`,`NewPatient`,`ReturnPatient`)
                        VALUES ('2','$encounterDetails[pid]','$encounterDetails[encounterNr]','$inputDate'
...
function insertQuestions($encounterDetails,$inputTime,$inputDate,$inputUser)
{
    ...
    $diagDate=new DateTime($encounterDetails[DateDiagnosed]);
    $diagoseDate=$diagDate->Format('Y-m-d');

    $sql2 = "INSERT INTO `care_hha_questions` (`FormID`,`PID`,`EncounterNr`,`InputDate`,`Smoking`,`Drinking`,`Cardiovascular`,`Diabetes`,
                              `Diet`,`Physical`,`HtnStatus`,`YearDiagnosed`,`Observations`,
                              `LMP`,`DrugAllergies`,`AllergiesDetails`,`AdheringMedications`,`AdheringMedicationsDetails`,`FollowupPlan`
                              ,`Clinician`,`Designation`,`InputUser`)
                            VALUES('1','$encounterDetails[pid]','$encounterDetails[encounterNr]','$inputDate'
                            , '$encounterDetails[Smoking]','$encounterDetails[Drinking]','$encounterDetails[Cardiovascular]','$encounterDetails[Diabetes]'
                            ,'$encounterDetails[Diet]','$encounterDetails[Physical]','$encounterDetails[HtnStatus]','$diagoseDate'
                            ...
                            ,'$inputUser','$designation','$inputUser')";
...
function insertContinuationQuestions($encounterDetails,$inputTime,$inputDate,$inputUser)
{
    global $db;    //22295021
    $debug = false;
    $designation="Clinical Officer";
    ...
    $sql2 = "INSERT INTO `care_hha_questions` (`FormID`,`PID`,`EncounterNr`,`InputDate`,`Observations`,
...
function saveContinuationCare($encounterDetails) {
    ...
    $DOB = $dob->format('Y/m/d');
     ...
     if(validateEncounter($encounterDetails[encounterNr])<1){
         $error=insertContinuationVitals($encounterDetails,$inputTime,$inputDate,$input_User);
 
         $sql2 = "INSERT INTO `care_hha_questions` (`FormID`,`PID`,`EncounterNr`,`InputDate`,`Smoking`,`Drinking`,`Cardiovascular`,`Diabetes`,`Diet`,`Physical`,`Observations`,
                              `LMP`,`DrugAllergies`,`AllergiesDetails`,`AdheringMedications`,`AdheringMedicationsDetails`,`FollowupPlan`
                              ,`Clinician`,`Designation`,`InputUser`)
                            VALUES('1','$encounterDetails[pid]','$encounterDetails[encounterNr]','$inputDate'
                            , '$encounterDetails[Smoking]','$encounterDetails[Drinking]','$encounterDetails[Cardiovascular]','$encounterDetails[Diabetes]'
                            ,'$encounterDetails[Diet]','$encounterDetails[Physical]'
                            ,'$encounterDetails[Observations]','$encounterDetails[LMPfemale]','$encounterDetails[DrugAllergies]','$encounterDetails[AllergiesSpecify]'
                            ,'$encounterDetails[AdheringMedication]','$encounterDetails[AdheringSpecify]','$encounterDetails[FollowupPlan]'
                            ,'$input_User','$designation','$input_User')";
    ...
    if($error==0){
        ...
        echo "{failure:true,'error':'A Record already Exists for todays Encounter'}";
    }else if($error==3){
        echo "{failure:true,'error':'There was a problem with you data, Please Contact System Admin:<br>'.$sql2}";
    }
