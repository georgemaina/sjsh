...
if (!empty($externalcall))
    $smarty->assign('sToolbarTitle', "PID: ".$sTitleNr);
else
    $smarty->assign('sToolbarTitle', "$page_title (PID: $sTitleNr)");
...
if (!isset($printout))
    if ($mode == 'new')
        if ($show == 'insert')
            ...
            $smarty->assign('pbHelp', "javascript:gethelp('prescription_create.php','Prescription :: Create new records','" . $help_site . "')");
    else
        $smarty->assign('pbHelp', "javascript:gethelp('prescription.php','Prescription :: Overview','" . $help_site . "')");

$smarty->assign('breakfile', $breakfile);
$smarty->assign('breakfile',$_SESSION['backpath_diag']);
# Window bar title
...
//echo 'Discharge status '.$is_discharged;
...
if ($death_date && $death_date != DBF_NODATE) {
    ...
    $smarty->assign('sDeathDate', @formatDate2Local($death_date, $date_format,$sep_chars));
}
...
if ($mode == 'show' /* && !isset($externalcall) */) {
    ...
    $tbg = 'background="gui/img/common/' . $theme_com_icon . '/' . $bgimg . '"';
    if ($rows ) {
        ...
        if (!empty($externalcall)) {
            if (!isset($printout) ) {
                if($encounter_obj->Is_Discharged($pn)){
                    $strDis='true';
                }else{
                    // $strDis='false';
...
//            $smarty->assign('sPromptLink', '<a href="' . $thisfile . URL_APPEND . '&disablebuttons=' . $disablebuttons . '&pid=' . $_SESSION['sess_pid'] . '&target=' . $target . '&mode=new">' . $LDEnterNewRecord . '</a>');
//        History
//        $smarty->assign('sPromptLinkEdit', '<input type=submit value="show history" name="show_history" onclick="javascript:alert("Test Test");"/>');
        ob_end_clean();
...
//        }
        ...
        $encNo= $_SESSION['sess_en'];
         if($diagnosis==0){
            $diagnosisErr= "There is no Diagnosis entered for ".$title.' '.$name_last.": Click Here to Enter Diagnosis";
         }
        $notesError="There are no Notes entered for $title: Click Here to Enter Patient Notes";
        $vitalsError="No Vitals have been entered for $title: Click Here to Enter Vital Signs";
...
//$_SESSION['backpath_diag']
if (empty($externalcall)) {
    $smarty->assign('pbBottomClose', '<a href="' . $_SESSION['backpath_diag'] . '"><img ' . createLDImgSrc($root_path, 'close2.gif', '0') . '  title="' . $LDCancel . '"  align="absmiddle"></a>');
