...
<script language="javascript" >
	...
	urlholder="'.$root_path.'modules/nursing/nursing-station-patientdaten.php'.URL_REDIRECT_APPEND;
            ...
            echo '";';
            echo '
	patientwin=window.open(urlholder,pn,"width=1200,height=600,menubar=no,resizable=yes,scrollbars=yes");
	';
        }
        /*else echo '
...
if($data_entry){
    ...
    $TP_LABTESTS="<font color='#333333'>$LDLabtests</font>";
}

$jobid=$lab_obj->GetJobID($_SESSION['sess_en']);

if($data_entry){
    //$TP_LABRESULTS="<a href=\"show_labresults.php".URL_APPEND."&pid=$pid&target=$target&labTitle=Laboratory Tests\">$LDLabResults</a>";
    $TP_LABRESULTS="<a href='".$root_path."modules/laboratory_tz/labor_datalist_noedit.php".URL_APPEND."&encounter_nr=".$_SESSION['sess_en']."noexpand=1&nostat=1&user_origin=lab' title='$LDClk2See'>$LDLabResults</a>";
}else{
    $TP_LABRESULTS="<font color='#333333'>$LDLabResults</font>";
}

if($data_entry){
    $TP_XRAYTEST="<a href=".$root_path.'modules/nursing/nursing-station-patientdaten-doconsil-radio.php'.URL_REDIRECT_APPEND.'&station='.$station.'&pn='.$_SESSION['sess_en'].'&user_origin='.$user_origin.'&target=chemlabor&noresize=1&edit='.$edit.'">'.$LDXraytests."</a>";
}else{
    $TP_XRAYTEST="<font color='#333333'>$LDXraytests</font>";
}

if($data_entry){
    $TP_NOTESREPORTS="<a href=\"show_notes.php".URL_APPEND."&pid=$pid&target=$target&backpath_diag=".urlencode($_SERVER["PHP_SELF"].URL_APPEND.'&encounter_nr='.$encounter_nr)."\">$LDNotes $LDAndSym $LDReports</a>";
}else{
    $TP_NOTESREPORTS="<font color='#333333'>$LDNotes $LDAndSym $LDReports</font>";
}
//*********************************************************************************************************************
//Begin New Notes module
//*********************************************************************************************************************
if($data_entry){
    $TP_NOTESREPORTS2="<a href=\"../Treatment/index.html".URL_APPEND."&pid=$pid&target=$target\">$LDNotes2 $LDAndSym $LDReports2</a>";
}else{
    $TP_NOTESREPORTS2="<font color='#333333'>$LDNotes2 $LDAndSym $LDReports2</font>";
}
//*********************************************************************************************************************
//end of new Notes
//*********************************************************************************************************************
...
# Links to admision history generator
if($data_entry){
    $TP_ADMISSION_HISTORY="<a href=\"".$root_path."modules/registration_admission/patientAdmissionHistory.php".URL_APPEND."&pid=".$pid."&enc=".$_SESSION['sess_en']."\" target=_blank>Person Admission History</a>";
}else{
    $TP_ADMISSION_HISTORY="<font color='#333333'>$LDPrintPDFDoc</font>";
}

# Links to Nurses Cardedx generator
if($data_entry){
    $TP_NURSES_CARDEX="<a href=\"".$root_path."modules/registration_admission/patientNursesCardex.php".URL_APPEND."&pid=".$pid."&enc=".$_SESSION['sess_en']."\" target=_blank>Nurses Cardex</a>";
}else{
    $TP_NURSES_CARDEX="<font color='#333333'>$LDPrintPDFDoc</font>";
}
