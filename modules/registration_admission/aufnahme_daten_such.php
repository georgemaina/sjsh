	...
	if($mode!='paginate'){
		...
		if(is_numeric($suchwort)) {
			...
			$sql='SELECT enc.encounter_nr, enc.encounter_class_nr, enc.is_discharged,
								reg.pid, reg.name_last, reg.name_first,reg.name_2, reg.date_birth, reg.selian_pid,reg.sex, reg.selian_pid '.$dbtable.$sql2;
...
if($mode=='search'||$mode=='paginate'){
	...
	if ($linecount) {
		...
		$smarty->assign('LDFirstName',$pagen->makeSortLink($LDFirstName,'name_first',$oitem,$odir,$targetappend));
                $smarty->assign('LDName3',$pagen->makeSortLink($LDName3,'name_2',$oitem,$odir,$targetappend));
		...
		while($zeile=$ergebnis->FetchRow()){
			...
			$smarty->assign('sFirstName',ucfirst($zeile['name_first']));
                        $smarty->assign('sName3',ucfirst($zeile['name_2']));
			#
			...
			$smarty->assign('sBday',$zeile['date_birth']);
