...
<script> //display datagrid
    ...
    function showPrescriptions(str,pid){
        ...
        w1 = dhxWins.createWindow("w1", 800, 100, 540, 350);

        w1.setText("Prescription");
        grid = w1.attachGrid();
        grid.setImagePath("codebase/imgs/");
        grid.setHeader("PID,order_no,presc_nr,item_id,item_desc,price,qty,unit_cost,issued,orign_qty,balance");
        ...
        grid.setInitWidths("80,60,60,60,180,60,60,60,60,60,60");
        grid.loadXML("Prescription_conn.php?issDate="+str+"&pid="+pid);
//        grid.attachEvent("onRowSelect",doOnRowSelected1);
    ...
    function doOnRowDoubleClicked(id,ind){
        ...
        sgrid.cells(sgrid.getSelectedId(),4).setValue(grid.cells(id,5).getValue());
        sgrid.cells(sgrid.getSelectedId(),5).setValue(grid.cells(id,8).getValue());
        sgrid.cells(sgrid.getSelectedId(),6).setValue(grid.cells(id,7).getValue());

        document.getElementById('order_no').value=grid.cells(id,1).getValue();
