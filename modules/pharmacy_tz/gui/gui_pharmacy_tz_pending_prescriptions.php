...
<script language="JavaScript" src="<?php echo $root_path;?>js/tooltips.js"></script>
    <script>
        function getPendingPrescriptions(){
            alert('test '+document.getElementById('encounterClass').value);
        }
...
if ($back_path!='billing' || $comming_from=='pharmacy') {
	echo '<a href="'.URL_APPEND.'&target=search&task=newprescription&back_path='.$back_path.'&pharmacy=yes">
            <img src="../../gui/img/common/default/createnewpres_tz.gif" border=0 align="absmiddle" 
            alt="Create new Prescription"></a><br>';
    //onchange='javascript:submit_form('".$thisfile.URL_APPEND."&prescrServ=prescr&comming_from=pharmacy&type=".document.getElementById('encounterClass').value."')'
    echo "<form name='prescription' method='POST' action='".$thisfile.URL_APPEND."&mode=new&encounterClass=".$enclass."'>
        <br>Filter By: <Select id='encounterClass' name='encounterClass' onchange=this.form.submit()>
