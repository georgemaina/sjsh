...
//    sgrid.submitOnlyChanged(false);
    ...
    kgrid.setHeader("item_id,Item_desc,Unit_qty,Units_per_Pack,qty,qty_issued,Balance,Qty_In_Store,TotalUnits");
    kgrid.setInitWidths("80,200,80,100,60,80,60,120,80");
    kgrid.setSkin("light");
    kgrid.setColTypes("ed,ed,ed,ed,ed,ed,ro,ro,ro");
    kgrid.setColSorting("str,str,str,str,str,str,str,str,str");
...
function getsOrder(suppStore){
   ...
   function doOnCellEdit(stage,rowId,cellInd){
        ...
        var qty=kgrid.cells(kgrid.getSelectedId(),4).getValue();

        var strNum =kgrid.cells(kgrid.getSelectedId(),2).getValue();
        if(get_numbers(strNum)!=null){
            var unitQty=strNum;
        }else{
            var unitQty=1;
        }

        var qtyServ=kgrid.cells(kgrid.getSelectedId(),5).getValue();
        var bal=qty-qtyServ;
        var totalUnits=qty;
        if(stage==2 && cellInd==5){
//            kgrid.cells(rowId,5).setValue(total);
            if(qtyServ>qty){
                  alert("Quantity Serviced cannot be greater than Quantity Ordered");
				   kgrid.cells(rowId,5).setValue(0);
            }else{
            	   kgrid.cells(rowId,6).setValue(bal);
                   kgrid.cells(rowId,8).setValue(totalUnits);
...
//
    ...
    function sumColumn(ind){
        var out = 0;
        for(var i=0;i<kgrid.getRowsNum();i++){
            out+= parseFloat(kgrid.cells2(i,ind).getValue())
        }
        return out;
    }
