...
function getAdjustments() {
  ...
  // echo $sql;
    ...
    echo '<table width=100%  id="pharmacy"><tr>
                    ...
                    <th align="center" class=titles>User</th>
...
function getIssues() {
    ...
    if ($rstatus=='Pending') {
        $sql="SELECT nr as presc_nr,r.status,r.create_time as order_date,'' as store_loc,'' as store_desc,p.`pid` as OP_no,
                ...
                r.`partcode` as item_id,r.`article` as Item_desc,(r.`dosage`*r.`times_per_day`*r.`days`) AS qty,r.`price`,r.`prescriber` as ,e.`insurance_ID`,
                IF(p.`insurance_ID`='-1','CASH',c.`name`) AS PaymentMethod 
        ...
        $sql = "SELECT  o.order_no , o.presc_nr , o.status , o.order_date , o.order_time , o.store_loc , o.store_desc ,
                ...
                , o.balance , o.input_user , p.prescriber,o.total,e.`insurance_ID`,
                 IF(e.`insurance_ID`='-1','CASH',c.`name`) AS PaymentMethod 
                FROM  care_ke_internal_orders o 
                LEFT JOIN care_encounter_prescription p ON o.presc_nr=p.nr
                LEFT JOIN care_person e ON o.`OP_no`=e.`pid`
                LEFT JOIN care_tz_company c ON e.`insurance_ID`=c.`id`
...
//
    echo '<table  id="pharmacy"><tr>
                    ...
                    <th align="center" class=titles>Prescribed By</th>
                    <th align="center" class=titles>Issued By</th>
                    <th align="center" class=titles>Payment Method</th></tr>';
    $bg = '';
    $total=0;
    while ($row = $request->FetchRow()) {
//        if ($bg == "silver")
...
//            $bg = "silver";
        ...
        $total=$total+($row[orign_qty] * $row[price]);
        $rowbg = 'white';
    }
    echo "<tr><td colspan='13' align='rignt' class=titles>Total Cost</td><td class=titles>".number_format($total,2)."</td></tr>";
    echo '</table>';
}
?>
<!--
