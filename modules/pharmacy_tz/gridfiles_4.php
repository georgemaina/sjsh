<script>
    ...
    sgrid.setSkin("light");
    sgrid.setColTypes("ch,ro,ro,ro,ed,ed,ed,ed,ed,ed,ed,ro");
...
//           document.getElementById('totalCost').value=totals;
    ...
    function getReceiptNo(pid,encounter_nr){
        xmlhttp=GetXmlHttpObject();
        if (xmlhttp==null)
        {
            alert ("Browser does not support HTTP Request");
...
function applyFilter1(){
        ...
        gridQString = "issue_conn.php?patientId="+document.getElementById("pid").value+"&prescDate="+document.getElementById("issDate").value
		+"&storeID="+document.getElementById("storeID").value;//save query string in global variable (see step 5 for details)
...
//            alert(str);
            ...
            getReceiptNo(document.getElementById("pid").value,document.getElementById("enc_no").value);
        }
    }
    ...
    function doOnCellEdit(stage,rowId,cellInd){
        ...
        var dose=sgrid.cells(sgrid.getSelectedId(),4).getValue();
        var times=sgrid.cells(sgrid.getSelectedId(),5).getValue();
        var days=sgrid.cells(sgrid.getSelectedId(),6).getValue();

        var qty=sgrid.cells(sgrid.getSelectedId(),9).getValue();
        var amnt=sgrid.cells(sgrid.getSelectedId(),8).getValue();
        var issued=sgrid.cells(sgrid.getSelectedId(),10).getValue();
        var desc=sgrid.cells(sgrid.getSelectedId(),3).getValue();

        if(issued==""){
            var qtyIssued=qty;
        }else{
             var qtyIssued=issued;
        }
        

        var total=amnt*qtyIssued;

         if(stage==2 && cellInd==6 || cellInd==5){
            var qtyPres=dose*times*days;
            sgrid.cells(rowId,9).setValue(qtyPres); 
         }


        if(stage==2 && cellInd==10 || cellInd==8){

             if(issued>qty){
                alert("Qty Issued cannot be greater than Qty Prescribed");
                 document.getElementById('message').innerHTML="Qty Issued for "+ desc +" cannot be greater than Qty Prescribed";
                 sgrid.cells(rowId,10).setValue("");
             }else{
                 sgrid.cells(rowId,11).setValue(total);
                 document.getElementById('totalCost').value=sumColumn(11);
...
//        return out;
    ...
    function getPrescriptionHistory(pid){
        ...
        grid.setHeader("Date,status,partcode,article,dosage,times_per_day,days, quantity,amnt,qty,total");
        ...
        grid.setInitWidths("80,60,60,180,60,60,60,60,60,60,60");
        grid.loadXML("prescConn.php?pid="+pid);
...
//                }
//            };
//            var url = "pharmVals.php?desc10=" + str;
//            url = url + "&sid=" + Math.random();
