...
if($pharmLoc<>"care"){
    ...
    $sql = "SELECT a.nr,b.encounter_nr,b.encounter_class_nr, a.create_time as prescribe_date,a.partcode,b.pid,a.article_item_number,a.article,
            ...
            WHERE b.pid='".$pid."' AND a.drug_class in ('drug_list','Medical-Supplies') and b.encounter_class_nr=2
            AND a.status NOT IN('serviced','done','Cancelled') and a.prescribe_date='$prescDate'";

       //echo $sql;
      //if(isset($pid))
     //  $sql.=" Where `IP-OP`='1' and pid ='".$pid."'";

    $gridConn->render_sql("$sql","nr", "article_item_number,prescribe_date,partcode,article,dosage,times_per_day,days,quantity,amnt,qty,issued,total");
