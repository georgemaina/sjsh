...
var itemCat2=new Ext.data.JsonStore({
    url: 'getDrugs.php?task=getStores',
    root: 'stores',
    id: 'catID',
    autoLoad: true,
    fields:['Store']
...
var itemsReader=new Ext.data.JsonStore({
    ...
    fields:['ID','item_number','loccode','item_Description','quantity', 'reorderlevel','item_cat','comment']
});
...
function handleDelItem() {
    ...
    var sItem=itemsGridForm.getForm().findField('ID').value
    if(sItem.length > 0)
    {
...
function deleteItem(btn) {
    var sItem=itemsGridForm.getForm().findField('ID').value
    var sStore=itemsGridForm.getForm().findField('loccode').value
    if(btn=='yes')
    {
        //submit to server
        ...
        {   //specify options (note success/failure below that receives these same options)
            ...
            params: {
                task: "deleteLevelItem", //pass task to do to the server script
