...
switch ($task) {
    ...
    case "create":
        ...
        break;
    case "adjustStock":
        stockAdjust($item_number,$input_user,$drugs_obj);
        break;
    case "deleteItem":
    ...
    case "getQuantityByItem":
        getQuantityByItem($partCode,$storeID);
        break;
    default:
        echo "{failure:true}";
        break;
...
function getStores(){
    ...
    $sql="SELECT DISTINCT loccode FROM care_ke_locstock";
    if($debug) echo $sql;
...
function getQuantityByItem($partCode,$storeID){
    global $db;
    $debug=false;
...
function stockAdjust($item_number,$input_user,$drugs_obj) {
    global $db;
    $item_number = $_REQUEST[item_number];
    $item_description = $_REQUEST[item_Description];
	...
	values( '" . $item_number . "', '" . $row[0] . "', '" . $qty . "', '$input_user',
            '" . date('Y-m-d') . "', '" . date('H:i:s') . "', '" . $adjDesc . "','$loccode')";
...
//        echo $sql;
        $db->Execute($sql);
         ...
         updateStockMovement($item_number,$loccode,$qty,$currQty,$input_user,$drugs_obj);
...
function updateStockMovement($stockid,$store_loc,$qty,$currQty,$input_user,$drugs_obj){
...
//    $currQty=getCurrentQty($stockid,$store_loc);
    ...
    $moveDetails[pid]='';
    $moveDetails[price]=0;
    $moveDetails[reference]="Stock Adjustment in $store_loc from $currQty to $qty";
    $moveDetails[qty]=$newqoh;
    $moveDetails[standardcost]=0;
    $moveDetails[newqoh]=$qty;
    $moveDetails[narrative]="Stock Adjustment in $store_loc from $currQty to $qty";
...
function getLevels($billObj,$limit, $start) {
    global $db;
    $debug=false;
    $catID = $_POST['catID'];
    $itemName = $_POST['itemName'];
    $loccode=$_REQUEST['loccode'];
        ...
        from care_tz_drugsandservices b  INNER join care_tz_itemscat e on b.category=e.catid
        INNER join care_ke_locstock k on k.stockid=b.partcode where b.item_description like '%$itemName%'";

        if($loccode<>''){
            $sql.=" and k.loccode='$loccode'";
        }

       $sql=$sql." and k.loccode ='DISPENS'  limit $start,$limit";
  if($debug) echo $sql;
    $result = $db->Execute($sql);
    ...
    while ($row = $result->FetchRow()) {
        ...
        $fDesc = addslashes($row[6]);
        $itmNo = addslashes($row[0]);
       // $fDesc1 = preg_replace('/[^a-zA-Z0-9_ -]/s', '', $row[2]);
		$fDesc1 = $billObj->escapeJsonString($row[2]);
