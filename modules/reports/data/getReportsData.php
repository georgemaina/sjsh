...
switch ($task) {
    case "updateOpWorkload":
        updateopWorkload($startDate, $endDate);
        break;
    case "getDiagnosis":
        getDiagnosis();
        break;
...
function getOPInvoices($startDate,$endDate,$pid){
    ...
    while ($row = $results->FetchRow()) {
        ...
        echo '{"Accno":"' . $row['accno'].'","MemberNo":"' . $row['memberID'].'","Pid":"' . $row['pid'].'","EncounterNo":"' . $row['encounter_nr']
            .'","Names":"' . $row['pnames'].'","BillNumber":"' . $row['bill_number']
                .'","Amount":"' . number_format($row['Total'],2).'","BillDate":"' . $row['bill_date']
            .'","PaymentMethod":"' . $row['PaymentMethod'].'","Diagnosis":"' . $diagnosis. '"}';
        $counter++;
        if ($counter < $total) {
            echo ",";
        }
...
function updateopWorkload($startDate,$endDate){
    global $db;
...
function getOPvalues($age, $sex, $clinic, $sign, $encDate1, $encDate2, $sign2) {
    ...
    if ($sign2 == "=") {
        ...
        $sql = "select count(e.encounter_nr) as encCounter,e.pid from care_encounter e 
                INNER JOIN care_person p on e.pid=p.pid
                INNER JOIN care_encounter_location l ON e.`encounter_nr`=l.`encounter_nr`
                WHERE  l.location_nr='$clinic' and encounter_date between '$encDate1' and '$encDate2'";
...
function getOPvisits($date1,$date2) {
    ...
    $sql = "SELECT `Parent`,`OpCode`,`Description`,`ReportDate`,`ReportTime`,SUM(`New`) as newPatients,SUM(`Return`) AS returnPatients
...
function getNhifClaims($startDate,$endDate){
    ...
    $sql = "SELECT b.creditNo,b.inputDate,b.admno,b.Names,b.admDate,b.disDate,b.wrdDays,b.nhifNo,b.nhifDebtorNo,
        ...
        FROM care_ke_nhifcredits b WHERE n.rev_code='NHIF' ";
...
function getLabTests($startDate,$endDate){
    ...
    $sql="SELECT p.`group_id`,p.Item_Id,p.name AS Description,
    IF(n.`sex`='m',COUNT(l.`item_id`),0) AS Male,
    IF(n.`sex`='f',COUNT(l.`item_id`),0) AS Female,
    IF((YEAR(NOW())-YEAR(n.date_birth))<5,COUNT(l.`item_id`),0) AS below5,
    IF((YEAR(NOW())-YEAR(n.date_birth)) BETWEEN 5 AND 14,COUNT(l.`item_id`),0) AS between5and14,
    IF((YEAR(NOW())-YEAR(n.date_birth)) >14,COUNT(l.`item_id`),0) AS above14
     FROM `care_test_request_chemlabor_sub` l
                LEFT JOIN `care_test_request_chemlabor` s ON l.`batch_nr`=s.`batch_nr`
                LEFT JOIN care_encounter e ON l.`encounter_nr`=e.`encounter_nr`
                LEFT JOIN care_person n ON e.`pid`=n.`pid`
                LEFT JOIN `care_tz_laboratory_param` p ON l.`item_id`=p.`item_id`";

    if($startDate<>'' and $endDate<>''){
        $sql=$sql." WHERE s.send_date BETWEEN '$startDate' AND '$endDate'";
    }else{
        $sql=$sql." WHERE s.send_date BETWEEN '".date('Y-m-d')."' AND '".date('Y-m-d')."'";
    }

    $sql=$sql." GROUP BY l.`item_id`";
    ...
    while($row=$results->FetchRow()){
        ...
        echo '{"Group":"'.$row['group_id'].'","ItemID":"'.$row['Item_Id'].'","Description":"'.$row['Description']
              .'","Male":"'.$row['Male'].'","Female":"'.$row['Female'].'","Total":"'.$total
              .'","Below5":"'.$row['below5'].'","Between5And14":"'.$row['between5and14'].'","Above14":"'.$row['above14'].'"}';
...
function getXrayPatientTests($person) {
    global $db;
    $debug=false;
    $date1 = $_REQUEST['date1'];
    $date2 = $_REQUEST['date2'];
  ...
  WHERE b.service_type = 'xray' and b.pid=$pid";
   ...
   echo '{"totalCount":'.$rowCnt.',"xrayrevenue":[';
    ...
    while ($row = $request->FetchRow()) {
        ...
        $enc_class=$current_encounter['encounter_class_nr'];
        if($row['admission']=='1'){
            $adm='IP';
        }else{
            $adm='OP';
        }
        echo '{"xraycode":"' . $row['pid']  . '","Names":"' .$row['name_first'] . ' ' . $row['name_last'] . ' ' . $row['name_2']
...
function getWardsInfo($ward_obj){
    global $db;
    echo '{"wardinfo":[';
    $sql='SELECT nr FROM care_ward';
    $result = $db->Execute($sql);
    while($row = $result->FetchRow()){
        getWardDetails($row[0],$ward_obj);
    }
    echo ']}';
}

function getWardDetails($wrdNO,$ward_obj) {
    global $db;
    $db->debug = 0;
    require('./roots.php');
