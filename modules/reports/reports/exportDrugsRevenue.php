...
$worksheet->write(1, 1, 'Drugs Revenue By Category '.$ordLoc, $formatHeader);
...
// Create the header for the data starting @ row 4
    $worksheet->write($indexRow, $indexCol++, 'Category ID', $formatReportHeader);
    $worksheet->write($indexRow, $indexCol++, 'Description', $formatReportHeader);
    $worksheet->write($indexRow, $indexCol++, 'Total drugs', $formatReportHeader);
   ...
   $sql = "SELECT d.category,i.item_Cat,b.service_type, SUM(b.total) AS Total,COUNT(b.partcode) AS drug_count
FROM care_ke_billing b inner join care_tz_drugsandservices d
on b.partcode=d.partcode  inner join care_tz_itemscat i
on d.category=i.catID
WHERE b.service_type = 'drug_list' ";
    ...
    $sql = $sql . "  group by d.category order by i.item_Cat asc";
//    echo $sql;
