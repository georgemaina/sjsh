...
?>
<link rel="stylesheet" type="text/css" href="nursing.css">
<?php
...
switch ($mode) {
    case 'save':
        if (prepareTestElements()) {
            ...
            $data['item_id'] = $pres_obj->GetItemIDByNumber('LAB' . $value);
...
//                    $item_id = $prescription_obj->GetItemIDByNumber($rowr[0]);
                    // TODO: Set here the function to store it as bill - element -> drug_class='lab'
                    $pres_obj->insert_prescription($pn, $rowr[0],$type);
...
//                                createlabQuote($batch_nr,$pn,$breakfile,$root_path);
                ...
                $result = $bill_obj->GetNewQuotation_Laboratory($pn, $in_op,$id_array);
                if (!$result) {
                ...
                if ($weberp_obj = new_weberp()) {
                    ...
                    while ($row = $result->FetchRow()) {
                        ...
                        if (!$weberp_obj->transfer_bill_to_webERP_asSalesInvoice($row)) {
                            ...
                            $db->Execute($sql);
                            $sql2 = "update care_ke_billing set weberpSync=1 where weberpSync=0 and bill_number='$row[bill_number]'";
                            $db->Execute($sql2);
                        } else {
...
<ul>
    ...
    <?php
            ...
            //$accno=$bill_obj->getBillPaymentMethod($pn);
?> <br>
...
?> <!--  Table for the day and month code -->
				 ...
				 <tr><td class="billMessage"  colspan="2">The Payment Method is  </td>';
                    echo '<td class="billMsgValue" colspan="6" align="left">'.$billing_tz->getBillPaymentMethod($h_pid).'';
				   echo '</td></td></tr>
				<tr>';
