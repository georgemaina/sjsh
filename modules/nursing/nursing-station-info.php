    ...
    case 'update': {
            ...
            if ($ward_obj->updateWard($ward_nr, $_POST)) {
                ...
                foreach ($roomBedsArr as $careRoom => $anzahl) {
                    ...
                    //print_r($dataArrRomAnzahl);
                    $ward_obj->updateRoomNumberBeds($dataArrRoomAnzahl);
                }

                //update number of the first Bed in the Room
                foreach ($firstBedNoArr as $careRoom => $firstBed) {
                    $dataArrRoomBedNo = array('nr' => $careRoom, 'firstbedno' => $firstBed);
                    //print_r($dataArrRomAnzahl);
                    $ward_obj->updateFirstBedno($dataArrRoomBedNo);
                }

                //update number of of the Last Bed in the Room
                foreach ($lastBedNoArr as $careRoom => $lastBed) {
                    $dataArrRoomBedNo = array('nr' => $careRoom, 'lastbedno' => $lastBed);
                    //print_r($dataArrRomAnzahl);
                    $ward_obj->updateLastBedNo($dataArrRoomBedNo);
                }
                ...
                //update room charge
                foreach ($roomCostArr as $careRoom => $cost){
                    $dataArrRoomCharge = array('nr' => $careRoom, 'charge' => $cost);
                    $ward_obj->updateRoomCharge($dataArrRoomCharge);
                }
                //update nursing fee 'charge2'
                foreach ($roomCostArr2 as $careRoom => $cost){
                    $dataArrRoomCharge2 = array('nr' => $careRoom, 'charge2' => $cost);
                    $ward_obj->updateRoomCharge2($dataArrRoomCharge2);
                }
                //update Doctors fee 'charge3'
                foreach ($roomCostArr3 as $careRoom => $cost){
                    $dataArrRoomCharge3 = array('nr' => $careRoom, 'charge3' => $cost);
                    $ward_obj->updateRoomCharge3($dataArrRoomCharge3);
                }

                //update Accommodation 'charge4'
                foreach ($roomCostArr4 as $careRoom => $cost){
                    $dataArrRoomCharge4 = array('nr' => $careRoom, 'charge4' => $cost);
                    $ward_obj->updateRoomCharge4($dataArrRoomCharge4);
                }
...
if ($rows == 1) {
    ...
    $smarty->assign('date_create', $date_create);
    ...
    if (is_object($rooms)) {
        ...
        $smarty->assign('LDFirstBedNo', $LDFirstBedNo);
        $smarty->assign('LDLastBedNo', $LDLastBedNo);
        $smarty->assign('LDRoomShortDescription', $LDRoomShortDescription);
        $smarty->assign('LDRoomCharge', $LDRoomCharge);
        $smarty->assign('LDNursingCharge', $LDNursingCharge);
        $smarty->assign('LDDoctorsCharge', $LDDoctorsCharge);
        $smarty->assign('LDAccomCharge', $LDAccomCharge);
        $roomCount = 0;
        $toggle = 0;
        $sTemp = '';
        while ($room = $rooms->FetchRow()) {
            ...
            $sTemp = $sTemp . '
				...
				<td class="room"><input type="text" name="roomBedsArr[' . $room['nr'] . ']" value="'. $room['nr_of_beds'].'"></td>
				<td class="room"><input type="text" name="firstBedNoArr[' . $room['nr'] . ']"  id="firstBedNoArr[' . $room['nr'] . ']" value="'. $room['firstBedNo'].'"></td>
				<td class="room"><input type="text" name="lastBedNoArr[' . $room['nr'] . ']"  id="lastBedNoArr[' . $room['nr'] . ']" value="'. $room['lastBedNo'].'"></td>
				...
				<td class="room"><input type="text" name="roomCostArr[' . $room['nr'] . ']" id="roomCostArr[' . $room['nr'] . ']" value="' . $room['charge'] . '"></td>
				<td class="room"><input type="text" name="roomCostArr2[' . $room['nr'] . ']" id="roomCostArr2[' . $room['nr'] . ']" value="' . $room['charge2'] . '"></td>
				<td class="room"><input type="text" name="roomCostArr3[' . $room['nr'] . ']" id="roomCostArr3[' . $room['nr'] . ']" value="' . $room['charge3'] . '"></td>
				<td class="room"><input type="text" name="roomCostArr4[' . $room['nr'] . ']" id="roomCostArr4[' . $room['nr'] . ']" value="' . $room['charge4'] . '"></td>
                                </tr>';
        }
    ...
    if ($ward['is_temp_closed']) {
        ...
        <form name="closer" method="post" action="<?php echo $thisfile ?>" onSubmit="return checkReopen()" onReset="return checkClose(this)">
            ...
            <input type="hidden" name="charge" value="<?php echo $ward['charge'] ?>">
            <input type="hidden" name="charge2" value="<?php echo $ward['charge2'] ?>">
            <input type="hidden" name="charge3" value="<?php echo $ward['charge3'] ?>">
        ...
        <form name="closer" method="post" action="<?php echo $thisfile ?>" onSubmit="return checkTempClose()" onReset="return checkClose(this)">
            ...
            <input type="hidden" name="sid" value="<?php echo $sid ?>">
            <input type="hidden" name="lang" value="<?php echo $lang ?>">
            <input type="hidden" name="ward_charge" value="<?php echo $ward['charge'] ?>">
            <input type="hidden" name="ward_charge1" value="<?php echo $ward['charge2'] ?>">
            <input type="hidden" name="ward_charg3" value="<?php echo $ward['charge3'] ?>">
