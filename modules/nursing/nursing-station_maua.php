...
function getrem(pn){
	...
	patientwin=window.open(urlholder,pn,"width=1200,height=500,menubar=no,resizable=yes,scrollbars=yes");
	}
			...
			if($bed['date_birth']){
				...
				if(isset($sg)&&$sg) $smarty->assign('sBirthDate',eregi_replace($sg,"<font color=#ff0000><b>".ucfirst($sg)."</b></font>",formatDate2Local($bed['date_birth'],$date_format)));
					    ...
					    if($disTypes[0]<>9){
                            ...
                            $smarty->assign('sBirthDate','<span style="background:yellow">'.formatDate2Local($bed['date_birth'],$date_format).'</span>');
//                            $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
                        }
...
$smarty->assign('pbClose','<a href="'.$breakfile.'" onclick="ConfirmClose()"><img '.createLDImgSrc($root_path,'close2.gif','0','absmiddle').'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp');
