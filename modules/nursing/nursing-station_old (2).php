...
//			$smarty->assign('sBed',strtoupper(chr($j+96)));
			...
			if($is_patient && ($bed['encounter_nr']!="")){
				...
				if(isset($sln)&&$sln) $sFamNameBuffer = preg_replace($sln,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_last']));
					else
					    if($disTypes[0]<>9){
				...
				if(isset($sfn)&&$sfn) $sNameBuffer = preg_replace($sfn,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_first']));
            ...
            if($disTypes[0]<>9){
                $smarty->assign('sName2',$bed['name_2']);
            }else{
                $smarty->assign('sName2','<span style="background:yellow">'.$bed['name_2'].'</span>');
//                $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
            }
			...
			if($bed['date_birth']){
				...
				if(isset($sg)&&$sg) $smarty->assign('sBirthDate',preg_replace($sg,"<font color=#ff0000><b>".ucfirst($sg)."</b></font>",$bed['date_birth']));
					else
					    if($disTypes[0]<>9){
                            $smarty->assign('sBirthDate',$bed['date_birth']);
                        }else{
                            $smarty->assign('sBirthDate','<span style="background:yellow">'.$bed['date_birth'].'</span>');
//                            $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
                        }
			...
			if ($bed['encounter_nr'])
			{
				...
				$time = $enc_obj->EncounterTime();

				$dateArr = date_parse($time);

				$smarty->assign('sPatNr',$pid);
				$smarty->assign('sAdmDate',$date.' '.$dateArr['hour'].':'.$dateArr['minute']);
...
//				else  $sBuffer = $sBuffer.$bed['insurance_name'];
			...
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
					...
					$smarty->assign('sInvoiceIcon','<a href="javascript:previewInvoice(\''.$pid.'\',\''.$billNumber.'\')"><img '.createComIcon($root_path,'show.gif','0','',TRUE).' alt="'.$LDOpenInvoice.'"></a>');
	...
	if($waitlist_count&&$edit){
		while($waitpatient=$waitlist->FetchRow()){
			...
			$TP_WLIST_BLOCK.='&nbsp;'.$waitpatient['name_last'].', '.$waitpatient['name_first'].' '.$waitpatient['date_birth'].'</nobr></a><br>';
		}
	}else{
		$TP_WLIST_BLOCK='&nbsp;';
	}
