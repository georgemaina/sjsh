		...
		if(prepareTestElements())
		{
			...
			if($diag_obj->insertDataFromInternalArray()){
				...
				//new code:
                                ...
                                $sqlInner2 = "SELECT * from care_tz_laboratory_param WHERE name='".$row1['name']."'";
...
//					echo 'item id='.$pre_item_id;
                                ...
                                $pres_obj->insert_prescription($pn,$pre_item_id,1);

				while (list($u,$v) = each ($param_array)) {
				 // Get for each lab-request-id the item_id of this lab-test out of drugsandservices-table
				 $item_id = $prescription_obj->GetItemIDByNumber('LAB'.$u);
				 // TODO: Set here the function to store it as bill - element -> drug_class='lab'
				 $prescription_obj->insert_prescription($pn,$pre_item_id);

				 } // end of while (list($u,$v) = each ($param_array))

                               // createlabQuote($pn,$batch_nr);
							...
							<table cellpadding="0" border="0" bgcolor="">
								<tbody>
									<tr>
										...
										<?php if($urgency==3) echo 'checked'; ?>><?php echo 'Priority'; ?></td>
										<td><input type="radio" name="urgency" value="5"
										<?php if($urgency==5) echo 'checked'; ?>><?php echo 'Urgent'; ?></td>
										<td><input type="radio" name="urgency" value="7"
										<?php if($urgency==7) echo 'checked'; ?>><?php echo 'Emergency'; ?></td>
										<td width=10%>&nbsp;</td>
