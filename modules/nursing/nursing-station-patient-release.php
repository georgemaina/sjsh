...
if( $enc_obj->loadEncounterData($pn)) {
	...
	if(($mode=='release')&&!(isset($lock)||$lock)){
		$date=date('Y-m-d'); //(empty($x_date))?date('Y-m-d'):formatDate2STD($x_date,$date_format,'Y-m-d');
...
//						break;
            case 9: $released=$enc_obj->DischargeMedically($pn,$relart,$date,$time);
            break;
...
//       if(d.bill_balance.value!=0 && d.insurance.value==''){
//           alert("The Patient cannot be Discharged, The is a Pending bill of Ksh."+ d.bill_balance.value) return false;
//       }
        ...
        if(d.relart[0].checked==true){
            if(!confirm("Are you sure you want to discharge "+"<?php echo $pn ?>") return false;
        }
//
		return true;
	}
...
//$bill_balance=$bill_obj->getBillBalance($enc_obj->PID(),$bill_obj->checkBillEncounter($pn));
$insurance=$insurance->Get_insuranceID_from_pid($enc_obj->PID());
...
if($bill_balance!=0){
    $smarty->assign('sBalance',$LDHasBalance);
}

//    $smarty->assign('sInsurance',$LDHasInsurance);
if($patient_ok){
	...
	if($released){
		...
		$smarty->assign('sDateInput','<input type="text" name="x_date" size=12 maxlength=10 value="'.formatdate2Local(date('Y-m-d'),$date_format,'Y-m-d',FALSE,'-').'"  onBlur="IsValidDate(this,\''.$date_format.'\')"  onKeyUp="setDate(this,\''.$date_format.'\',\''. $lang.'\')">');
	...
	$sTemp = $sTemp.'<input type="hidden" name="sid" value="'.$sid.'">
		...
		<input type="hidden" name="bill_balance" value="'.$bill_balance.'">
...
if(($mode=='release')&&($released)) $sSummaryButton= '<img '.createLDImgSrc($root_path,'printout.gif','0').'>';
	else $sSummaryButton= '<img '.createLDImgSrc($root_path,'printout.gif','0').' border="0">';
	
$smarty->assign('pbCancel','<a href="'.$breakfile.'">'.$sBreakButton.'</a>');
$smarty->assign('pbSummary','<a href='.$root_path.'modules/nursing/dischargeSummary.php?en='.$pn.'>'.$sSummaryButton.'</a>');
