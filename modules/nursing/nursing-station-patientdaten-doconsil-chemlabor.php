...
//            add_data_to_database($_POST);
            ...
            if (prepareTestElements()) {
                ...
                if ($diag_obj->insertDataFromInternalArray()) {
                    ...
                    foreach ($singleParam as $key => $value) {
                        ...
                        $parsedParamList['testStatus'] = $testStatus;
    ...
    //					echo 'item id='.$pre_item_id;
                    ...
                    $sql="INSERT INTO `care_test_request_status` (`encounter_nr`,`batch_nr`,`updateTime`,`status`,`statusDesc`,`statusType`,`inputby`) 
                          VALUES('$pn','$batch_nr','".date('Y-m-d H:i:s')."','Test Request Pending','Laboratory Test Request Sent Lab','Laboratory','".$_SESSION['sess_user_name']."')";
                        // echo $sql;
                        $db->Execute($sql);

                    $sql="update care_test_request_chemlabor_sub set TestStatus='Pending' where  batch_nr='$batch_nr'";
                    //echo $sql;
                    $db->Execute($sql);
    ...
    //                    $item_id = $prescription_obj->GetItemIDByNumber($rowr[0]);
                        // TODO: Set here the function to store it as bill - element -> drug_class='lab'
                        $pres_obj->insert_prescription($pn, $rowr[0],'lab');
...
$diagnosis=$diag_obj->checkCurrentEncounterDignosis($pn);
$notes=$diag_obj->checkCurrentEncounterNotes($pn);
$vitals=$diag_obj->checkCurrentEncounterVitals($_SESSION['sess_en']);
...
//        $smb="<a href=$_SESSION[backpath_diag]>
//                <img src='../../gui/img/control/default/en/en_back2.gif' border=0 width='110' height='24' alt=''
//                 style='filter:alpha(opacity=70)'></a>";
        $smb=$_SESSION['backpath_diag'];
        $smarty->assign('breakfile',$smb );
...
//     echo "Diagnosis is $diagnosis and $encounterClass and this $thisfile<br>";
//   if($diagnosis==0 && $encounterClass==2 && $thisfile == 'nursing-station-patientdaten-doconsil-chemlabor.php'){
//                
//                echo "<div style='font-size: large;font-weight:bold;color:maroon;'>There is no Diagnosis entered for $title: Click Here to Enter Diagnosis"
//                    . "<a href=\"../diagnostics_tz/icd10_quicklist.php".URL_REDIRECT_APPEND."&sid=".$sid
//                    ."&encounter=".$pn."&lang=en&ntid=false&externalcall=true&target=search&1=1&ispopup=false&backpath_diag="
//                    .urlencode($_SERVER["PHP_SELF"].URL_APPEND.'&encounter_nr='.$pn)."\"> <img " . createComIcon($root_path, 'createnew_tz.gif', '0') . " ></a></div>";
//    }elseif($notes==0 && $encounterClass==2){
//                echo "<div style='font-size: large;font-weight:bold;color:maroon;'>There are no Notes entered for $title: Click Here to Enter Patient Notes"
//                    . "<a href='".$root_path."modules/registration_admission/show_notes.php".URL_REDIRECT_APPEND."&station=$station&pn=$pn&user_origin=$user_origin&edit=$edit'><img " . createComIcon($root_path, 'createnew_tz.gif', '0') . "></a></div>";
//    }else{
	
//	if($vitals==0 && $encounterClass==2){
//		echo "<div style='font-size: large;font-weight:bold;color:maroon;'>There are no Vitals entered for $title: Click Here to Enter Patient Vitals"
//                    . "<a href='".$root_path."modules/registration_admission/show_weight_height.php". URL_APPEND."&pid=$pn&target=$target'><img " . createComIcon($root_path, 'createnew_tz.gif', '0') . "></a></div>";
//	}else{
        ?>
        ...
        //            alert(element.name+element.value);
            ...
            function getItemCosts(elem){
                ...
                var url="getData.php?task=getLabcost&financeClass=<?php $financialClass ?>";
                url=url+"&id="+elem;
                url=url+"&financeClass=1";
        ...
        <ul>
            <?php
//            $consultation=$billing_tz->checkConsultationPayment($_SESSION['sess_en']);
//    if($consultation==1){
            if ($edit) {
                ?>
                                                ...
                                                <td bgcolor="<?php echo $bgc1 ?>">
                                                    <table border=1 cellpadding=0 bgcolor="">
                                                        <tr>
                                                            <td><?php
                                                        if ($edit) {
                                                            $sql_headline = "SELECT p.pid,p.selian_pid, name_first,name_last,sex, e.encounter_nr,date_birth,n.notes
                                                               FROM care_person p, care_encounter e
                                                               LEFT JOIN care_encounter_notes n ON e.encounter_nr=n.encounter_nr
                                                               WHERE e.pid = p.pid AND e.encounter_nr ='" . $pn . "'";
                                                            ...
                                                            echo '</td>
                                                                ...
                                                                <tr><td color="#ffffee" class="vi_data" colspan="2">The Payment Method is '.$billing_tz->getBillPaymentMethod($h_pid).' </td>';
                                                                    echo "<td color='#ffffee' class='vi_data' colspan='6'>
                                                                          Clinical Information:<textarea name='notes' cols='25' rows='3'>$h_notes</textarea>
                                                                          Comments:<textarea name='comments' cols='25' rows='3'></textarea>";
        ...
        ?>
            ...
            ?>
                <input type="hidden" name="myform_key" value="<?php echo md5(date('Y:m:d H:i:s')); ?>" />
                    <input type="hidden" name="testStatus" value="Pending" />
                </form>
