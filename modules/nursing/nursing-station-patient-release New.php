...
if( $enc_obj->loadEncounterData($pn)) {
	...
	if(($mode=='release')&&!(isset($lock)||$lock)){
		$date=(empty($x_date))?date('Y-m-d'):formatDate2STD($x_date,$date_format,'Y-m-d');
...
function checkDischarge(encNo){
     ...
     xmlhttp=GetXmlHttpObject();
        ...
        var url="DischargeCode.php?encounterNo="+encNo;
...
function stateChanged()
 ...
 function disMessage(str){
     if(!confirm("Are you sure you want to discharge ")) return false;
 }
	...
	if(!d.sure.checked) {
       ...
       if(d.bill_balance.value!=0 && d.insurance.value==''){
        ...
        if(d.relart[0].checked==true){
            if(!confirm("Are you sure you want to discharge "+"<?php echo $pn.' and '.$bill_obj->getBillBalance($enc_obj->PID(),$bill_obj->checkBillEncounter($pn))  ?>")) return false;
        }
//                
		return true;
	}
...
if($patient_ok){
	...
	if($released){
		...
		$smarty->assign('sDateInput','<input type="text" name="x_date" size=12 maxlength=10 value="'.formatdate2Local(date('Y-m-d'),$date_format,'Y-m-d').'"  onBlur="IsValidDate(this,\''.$date_format.'\')"  onKeyUp="setDate(this,\''.$date_format.'\',\''. $lang.'\')">');
