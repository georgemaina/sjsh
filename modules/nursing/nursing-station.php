...
function previewInvoice(pid,billNumber)
{
		//alert('TEST '+pid+","+billNumber);
		urlholder="../accounting/reports/detail_invoice_pdf.php?pid="+pid+"&billNumber="+billNumber+"&receipt=1";
		transwin<?php echo $sid ?>=window.open(urlholder,"transwin<?php echo $sid ?>","width=650,height=600,menubar=no,resizable=yes,scrollbars=yes");	
}

function previewNotes(pid,encNo)
{
		//alert('TEST '+pid+","+billNumber);
		urlholder="../nursing/nurses_cardex.php?pid="+pid+"&encNo="+encNo+"&receipt=1";
		transwin<?php echo $sid ?>=window.open(urlholder,"transwin<?php echo $sid ?>","width=1000,height=1000,menubar=no,resizable=yes,scrollbars=yes");	
}

function previewDoctorsNotes(pid,encNo)
{
    //alert('TEST '+pid+","+billNumber);
    urlholder="../nursing/doctors_notes.php?pid="+pid+"&encNo="+encNo+"&receipt=1";
    transwin<?php echo $sid ?>=window.open(urlholder,"transwin<?php echo $sid ?>","width=1000,height=1000,menubar=no,resizable=yes,scrollbars=yes");
}
<?php
...
if($ward_ok){
	...
	$smarty->assign('LDOptions',$LDPatListElements[9]);
	...
	for ($i=$ward_info['room_nr_start'];$i<=$ward_info['room_nr_end'];$i++){
		...
		//for($j=1;$j<=$nr_beds;$j++){
			...
			if($j== $room_info['firstBedNo']){
				$smarty->assign('sRoom',strtoupper($room_info['info']).' Room '.$i);
			} else{
				$smarty->assign('sRoom','');
			}
...
//			$smarty->assign('sBed',strtoupper(chr($j+96)));
            ...
            $disTypes=$ward_obj->checkDischargeType($bed['encounter_nr']);
			...
			if($is_patient&&($bed['encounter_nr']!="")){
				...
				if(isset($sln)&&$sln) $sFamNameBuffer = eregi_replace($sln,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_last']));
					    ...
					    if($disTypes[0]<>9){
                            ...
                            $sFamNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_last']).'</span>';
                        }
				...
				if(isset($sfn)&&$sfn) $sNameBuffer = eregi_replace($sfn,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_first']));
					else if($disTypes[0]<>9){
                        $sNameBuffer = ucfirst($bed['name_first']);
                    }else{
                        $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
                    }
            ...
            if($disTypes[0]<>9){
                $smarty->assign('sName2',$bed['name2']);
            }else{
                $smarty->assign('sName2','<span style="background:yellow">'.$bed['name2'].'</span>');
//                $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
            }
			...
			if($bed['date_birth']){
				...
				if(isset($sg)&&$sg) $smarty->assign('sBirthDate',eregi_replace($sg,"<font color=#ff0000><b>".ucfirst($sg)."</b></font>",formatDate2Local($bed['date_birth'],$date_format,'FALSE','FALSE','-')));
					else
					    if($disTypes[0]<>9){
                            $smarty->assign('sBirthDate',formatDate2Local($bed['date_birth'],$date_format,'FALSE','FALSE','-'));
                        }else{
                            $smarty->assign('sBirthDate','<span style="background:yellow">'.formatDate2Local($bed['date_birth'],$date_format,'FALSE','FALSE','-').'</span>');
//                            $sNameBuffer = '<span style="background:yellow">'. ucfirst($bed['name_first']).'</span>';
                        }
			...
			if ($bed['encounter_nr'])
			{
				...
				$smarty->assign('sAdmDate',formatDate2Local($date,$date_format,'FALSE','FALSE','-')).' '.$dateArr['hour'].':'.$dateArr['minute'];
...
//				else  $sBuffer = $sBuffer.$bed['insurance_name'];
            ...
            if(($is_patient)&&($bed['encounter_nr']<>'')){
                $sBuffer=$bill_obj->getBillPaymentMethod($pid);
			...
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
					...
					$billNumber=$bill_obj->GetBill($bed['encounter_nr']);

                    ($bed['encounter_nr']=='')?'':$billIcon='<img '.createComIcon($root_path,'show.gif','0','',TRUE).' alt="'.$LDOpenInvoice.'">';
					$smarty->assign('sInvoiceIcon','<a href="javascript:previewInvoice(\''.$pid.'\',\''.$billNumber.'\')">'.$billIcon.'</a>');

					$smarty->assign('sNursesNotesIcon',$bed['encounter_nr'].'<a href="javascript:previewNotes(\''.$pid.'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'nurse.gif','0','',TRUE).' alt="'.$LDOpenNotes.'"></a>');
                    $smarty->assign('sNursesNotesIcon','<a href="javascript:previewDoctorsNotes(\''.$pid.'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'nurse.gif','0','',TRUE).' alt="'.$LDOpenNotes.'"></a>');
//                    $smarty->assign('sNursesNotesIcon','<a href="javascript:previewNotes(\''.$pid.'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'nurse.gif','0','',TRUE).' alt="'.$LDOpenNotes.'"></a>');
//                    $smarty->assign('sNursesNotesIcon','<a href="javascript:previewNotes(\''.$pid.'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'nurse.gif','0','',TRUE).' alt="'.$LDOpenNotes.'"></a>');
//                    $smarty->assign('sNursesNotesIcon','<a href="javascript:previewNotes(\''.$pid.'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'nurse.gif','0','',TRUE).' alt="'.$LDOpenNotes.'"></a>');
					
					$smarty->assign('sVitalsIcon','<a href="#"><img '.createComIcon($root_path,'people_search_online.gif','0','',TRUE).' id="vitals" alt="'.$LDOpenVitals.'"></a>');
					...
					$names=$bed['name_first']." ".$bed['name_2']." ".$bed['name_last'];
					
					$smarty->assign('sDischargeIcon','<a href="javascript:release(\''.$bed['room_nr'].'\',\''.$bed['bed_nr'].'\',\''.$bed['encounter_nr'].'\')"><img '.createComIcon($root_path,'bestell.gif','0','',TRUE).' 
							alt="'.$LDReleasePatient.'"></a>
							<input type="hidden" id="pid"  value="'.$bed['pid'].'">
							<input type="hidden" id="encounterNr" value="'.$bed['encounter_nr'].'">
	...
	if($waitlist_count&&$edit){
		while($waitpatient=$waitlist->FetchRow()){
			...
			$TP_WLIST_BLOCK.='&nbsp;'.$waitpatient['name_last'].', '.$waitpatient['name_first'].' '.formatDate2Local($waitpatient['date_birth'],$date_format,'FALSE','FALSE','-').'</nobr></a><br>';
		}
	}else{
		$TP_WLIST_BLOCK='&nbsp;';
	}
