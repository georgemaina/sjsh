...
$i=0;
$tests=$_POST[selected_item_list];
print_r($test);

foreach ($tests as $testsList){

    echo $testsList.',';

//foreach ($selected_item_list AS $itemsList){
    ...
    $sql="INSERT INTO care_test_request_".$db_request_table."
			...
			'".$if_patmobile."','".$if_allergy."','".$if_hyperten."','".$if_pregnant."',
			'".htmlspecialchars($clinical_info)."','".$testsList."','".date('Y-m-d')."',
...
function chkForm(d){
    ...
    selectBox = document.getElementById("selected_item_list[]");
...
?>    <tr><td><select name="itemlist[]" size="15" style="width:330px;" onDblClick="javascript:item_add();">
							...
							for ($i=0;$i<sizeOf($drug_list);$i++) {
								if ($drug_list[$i][2]==$stored_request['test_request']) {
									echo '<option selected value='.$drug_list[$i][drug_id].' selected>'.$drug_list[$i][description].'</option>';
								} else {
									echo '<option value='.$drug_list[$i][drug_id].' selected>'.$drug_list[$i][description].'</option>';
								}
							}
							?>
										</select>
	  ...
	  <td><select name="selected_item_list[]" multiple size="15" style="width:315px;" onDblClick="javascript:item_delete();"></select></td></tr>
	<tr>
      ...
      <td><input type="checkbox" name="nuclear" value="1" <?php if(($edit_form || $read_form) && $stored_request['nuclear']) echo "checked" ?>></td>
								...
								<tr>
									...
									<?php echo $LDPatMobile ?> &nbsp;<?php echo $LDYes ?>
										 <input type="radio" name="if_patmobile" value="1"
