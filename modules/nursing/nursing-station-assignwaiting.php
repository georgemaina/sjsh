...
if($encounter['death_date'] && $encounter['death_date'] != DBF_NODATE){
	...
	$smarty->assign('sDeathDate',@formatDate2Local($encounter['death_date'],$date_format,FALSE,FALSE,'-'));
}
...
$smarty->assign('sBdayDate',@formatDate2Local($encounter['date_birth'],$date_format,FALSE,FALSE,'-'));
...
//                          $smarty->assign('sBed',strtoupper(chr($j+96)));
			...
			if($is_patient&&($bed['encounter_nr']!="")){
				...
				if(isset($sln)&&$sln) $smarty->assign('sFamilyName',preg_replace($sln,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_last'])));
					else $smarty->assign('sFamilyName',ucfirst($bed['name_last']));

				if($bed['name_last']) $smarty->assign('cComma',',');
					else $smarty->assign('cComma','');

				if(isset($sfn)&&$sfn) $smarty->assign('sName',preg_replace($sfn,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_first'])));
			...
			if($bed['date_birth']){
				...
				if(isset($sg)&&$sg) $smarty->assign('sBirthDate',preg_replace($sg,"<font color=#ff0000><b>".ucfirst($sg)."</b></font>",formatDate2Local($bed['date_birth'],$date_format,false,false,'-')));
					else $smarty->assign('sBirthDate',formatDate2Local($bed['date_birth'],$date_format,false,false,'-'));
			}
