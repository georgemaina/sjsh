    ...
    case 'update': {
            ...
            if ($ward_obj->updateWard($ward_nr, $_POST)) {
                ...
                foreach ($roomCostArr as $careRoom => $cost){
                    $dataArrRoomCharge = array('nr' => $careRoom, 'cost' => $cost);
...
if ($rows == 1) {
    ...
    $smarty->assign('date_create', formatDate2Local($date_create, $date_format,'Y-m-d'));
...
//        $smarty->assign('LDLastBedNo', $LDLastBedNo);
        ...
        while ($room = $rooms->FetchRow()) {
            ...
            $sTemp = $sTemp . '
				...
				<td class="room"><input type="text" name="roomCostArr2[' . $room['nr'] . ']" id="roomCostArr3[' . $room['nr'] . ']" value="' . $room['charge2'] . '"></td>
