    ...
    function invoicePdf(name,receipt){
       ...
       var bill = document.bills.billNumbers.value;
        if(invType==1){
            ...
            window.open('summary_invoice_pdf.php?pid='+name+"&receipt="+str2+"&bill="+bill ,"Summary Invoice","menubar=no,toolbar=no,width=600,height=800,location=yes,resizable=yes,scrollbars=yes,status=yes");
        }
    }

function finalised(encounter_nr){
    ...
    function stateFinalised(){
          if (xmlhttp6.readyState==4)
        {
            ...
            if(str[0]==="1"){
                var pid= str[1];
                var bill = str[3];
            window.open('finalSummary_invoice_pdf.php?pid='+name+"&receipt="+str2+"&bill="+bill ,"Summary Invoice","menubar=no,toolbar=no,width=600,height=800,location=yes,resizable=yes,scrollbars=yes,status=yes");
   }else{
    ...
    function miniInvoicePdf(name,receipt){
        if(document.getElementById("receipt").checked==true){
            str2='ON';
        }else{
            str2='';
