...
$page->drawText("SUMMARY INVOICE", $leftPos + 45, $topPos - 10);
...
$page->drawText(date("Y-m-i H:i:s"), $leftPos + 40, $topPos - 70);
...
WHERE (pid ='" . $pid . "' AND service_type not in ('payment','payment adjustment','NHIF') and `ip-op`=1)
     and bill_number=$billNumber group by service_type";
...
while ( $row = $result->FetchRow()) {
    ...
    $page->drawText(substr($row['service_type'],0,30), $leftPos + 10, $topPos - $curr_point);
