...
//}
$sql5 = "SELECT * FROM care_ke_billing WHERE pid = '$q'";
$result5 = $db->Execute($sql5);
$row5=$result5->FetchRow();

$row2 = $wrd->EncounterLocationsInfo ( $row5 ['encounter_nr'] );
		$bed_nr = $row2 [6];
		$room_nr = $row2 [5];
		$ward_nr = $row2 [0];
		$ward_name = $row2 [1];
...
service_type NOT IN ('payment','payment adjustment') order by bill_date asc";
...
$sql2 = "SELECT sum(total) as total FROM care_ke_billing WHERE pid = '$q' and `IP-OP`=1 and service_type NOT IN ('payment','payment adjustment')";
...
$sqli = "SELECT * FROM care_ke_billing WHERE pid = '$q' and `IP-OP`=1 and service_type in ('payment','payment adjustment')";
...
while ($rowi = $resulti->FetchRow()) {
    ...
    echo "<tr><td >" . $rowi['bill_date'] . "</td>
       ...
       <td>Receipt No(".intval($rowi['batch_no']).")</td>
       <td>Ksh</td>
...
$sql2 = "SELECT sum(total) as total FROM care_ke_billing WHERE pid = '$q' and `IP-OP`=1 and service_type in ('payment','payment adjustment')";
...
if ($row2 = $result2->FetchRow()) {
    ...
    if($r<>''){
     ...
     echo " <tr><td colspan=5  align=right><b>Bill Balance:</b></td>
