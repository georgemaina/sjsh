...
WHERE (pid ='".$q."' AND service_type not in
    ('Payment','payment adjustment','NHIF') and `ip-op`=1) and bill_number=$billNumber group by service_type";
//echo $sql3;
...
if ($row2 = $result2->FetchRow()) {
    ...
    echo " <tr><td class=tbtopborder align=right><b>Total Bill:</b></td>
               <td class='tdbolder' align=left><b>Ksh." . number_format($row2['total'],2) . "</b></td></tr> ";
}

$sqli = "SELECT * FROM care_ke_billing WHERE pid = '$q' and `IP-OP`=1 and service_type 
in('NHIF') and bill_number=$billNumber";
$resulti = $db->Execute($sqli);
$rowi = $resulti->FetchRow();
$nhif=$rowi['total'];
if($nhif){
    echo "<tr><td >" . $rowi['bill_date'] . "
        Bill " . $rowi['Description'] . "(".intval($rowi['batch_no']).")</td>
...
$sqli = "SELECT * FROM care_ke_billing WHERE pid = '$q' and `IP-OP`=1 and service_type 
in('payment','payment adjustment') and bill_number=$billNumber";
...
if ($row2 = $result2->FetchRow()) {
    ...
    if($r=='ON'){
            echo " <tr><td align=right><b>Total Paid:</b></td>
                <td class='tdbolder' align=left><b>Ksh." . $row2['total'] . "</b></td></tr> ";
     }
     echo " <tr><td align=right><b>AMOUNT DUE:</b></td>
                            <td class='tdbolder' align=left><b>Ksh." ;
    if($r<>''){
        echo  number_format(intval($bill-$totalPaid),2);
    }else{
        echo number_format(intval($bill-$nhif),2);    
     }
    echo "</b></td></tr> ";
    }
