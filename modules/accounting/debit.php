...
//    jsIncludes();
    ...
    echo '</td><td width=80%>';
            ...
            if(!isset($_POST[submit])){
              ...
              //echo var_dump($_POST);0710148277
            
//                   $new_bill_number=$bill_obj->checkBillEncounter($_POST['en_nr']);
            
            $obj_acconts=new accounting();
//            
//            $sql='SELECT ward_id FROM CARE_WARD where ward_id='.$_POST[ward_nr];
////            echo $sql;
...
//            $row=$result->FetchRow();
            ...
            $row2 = $wrd->EncounterLocationsInfo2($_POST[en_nr]);
//            $bed_nr = $row2 [6];
//            $room_nr = $row2 [5];
//            $ward_nr = $row2 [0];
//            $ward_name = $row2 [1];
//            $adm_date = $row2 [7];
//            $dis_date = $row2 [8];
            $DebitDetails[ward_nr]=$row2 [0];
//            $DebitDetails[ward_id]=$row[0];
            $DebitDetails[ward_name]=$row2[1];
            $DebitDetails[room_nr]=$_POST[room_nr];
            $DebitDetails[bed_nr]=$row2[6];
            $DebitDetails[debitDate]=$_POST[calInput];//receiptNo
            $DebitDetails[receiptNo]=$_POST[receiptNo];
 ...
 function updateDbtErp($db,$pn) {
      ...
      $debug=false;;
        ...
        $sql='SELECT a.pid, a.total AS ovamount,a.partcode,a.description AS article,a.prescribe_date,
            a.qty AS amount,a.bill_number,a.ledger AS salesArea,a.batch_no
    FROM care_ke_billing a WHERE a.pid="'.$pn.'" AND weberpSync=0';
        if($debug) echo $sql;
        $result=$db->Execute($sql);
        ...
        //$arr=Array();
            ...
            //$weberp_obj = new_weberp();
                if(!$weberp_obj->transfer_bill_to_webERP_asSalesInvoice($row)) {
                    $sql="update care_ke_billing set weberpSync=1 where batch_no='$row[batch_no]' and partcode='$row[partcode]'";
                    if($debug) echo $sql;
                    $db->Execute($sql);
