...
//	$pers_obj->ResetPassword($userid,$pass);
    ...
    $sql="UPDATE care_users SET password='$pass', modify_id='".$_COOKIE[$local_user.$sid]."'  WHERE login_id='$userid'";
    if($debug) echo $sql;
    $db->Execute($sql);
}
...
<p>
<form method="post" action="edv_user_access_list.php" name="all">
...
foreach($area_opt as $x=>$v){
//while (list($x,$v)=each($area_opt))

   echo '<tr  bgcolor="white">';

   if (preg_match('/title/',$x))  // If title print it out
   {
