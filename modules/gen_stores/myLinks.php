...
<tbody class="main">
 ...
 <tr valign=top  class="titlebar" >
  <td bgcolor="#99ccff" >
    &nbsp;&nbsp;<font color="#330066">General Store Orders</font>
   ...
   href="../../modules/news/start_page.php?ntid=false&lang=$lang" ><img src="../../gui/img/control/default/en/en_close2.gif" border=0 width="103" height="24" alt="" style="filter:alpha(opacity=70)" onMouseover="hilite(this,1)" onMouseOut="hilite(this,0)"></a>  </td>
    ...
    echo '<table border="0" width="70%">
                <tbody>
                    <tr>
                        <td>Order Type</td>
                        <td colspan="3"><input type="radio"  id="orderType" name="orderType" value="Transfer" onclick="getNextIrnNo(this.value)"/>Transfere:
    ...
    echo '</select>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><select name="supStoreid" id="supStoreid" onchange="getStore2(this.value)">
                                        <option>Select Store</option>';
    $sql = "select st_id from care_ke_stlocation where st_id IN('GEN','KIT')";
    $result = $db->Execute($sql);
    while ($row = $result->FetchRow()) {
...
<tbody class="main">
 ...
 <table border="0" width="80%">
    ...
    $row = $result->FetchRow();
    $defaultStore = $row[0];
    $sql = "SELECT st_id,st_name FROM care_ke_stlocation where store=1  AND MainStore=1 and ID in($defaultStore)" ;
    $result = $db->Execute($sql);
...
function displayIssueDrugsForm($db) {
    echo '<table width=100% border=1 cellspacing=0 height=100%>
<tbody>
	<tr>
		<td  valign="top" align="middle" height="35">
			 <table cellspacing="0"  class="titlebar" border=0>
 <tr valign=top  class="titlebar" >
  <td bgcolor="#99ccff" >
    &nbsp;&nbsp;<font color="#330066">Issue Drugs</font>
  ...
  <td bgcolor="#99ccff" align=right><a
   ...
   href="../../modules/news/start_page.php?ntid=false&lang=$lang" ><img src="../../gui/img/control/default/en/en_close2.gif" border=0 width="103" height="24" alt="" style="filter:alpha(opacity=70)" onMouseover="hilite(this,1)" onMouseOut="hilite(this,0)"></a>  </td>
 ...
 </table> <form name="order" method="POST" action="' . $_SERVER['PHP_SELF'] . '"  onSubmit="return chkform(this)">
                <table width=80% border=0>
                    <tr>
                        <td>Type:</td>
                        <td><input type="radio" id="issType" name="issType" value="2" onclick="getNextIssNo(this.value)"/>:OutPatient
                    ...
                    <tr>
                        <td>Date</td>
                        <td><input type="text" id="issDate" name="issDate" value="' . $req_date = date("Y-m-d") . '" />
                            Issue No:<input type="text" id="issNo" name="issNo" value="" /></td>
                        <td>Cash Receipt:</td>
                        <td><input type="text" id="receiptNo" name="receiptNo" value="" /></td>
                    </tr>
                    <tr>
                        <td>Patient No</td>
                        <td><input type="text" id="pid" name="pid" value="" ondblclick="initPtsearch()" onblur="getOrders()"/>
                            <input type="text" id="pname" name="pname" value="" size="30" onclick="getOrders()" onblur="getDrugTotals(6);"/>
                            <input type="text" id="enc_no" name="enc_no" value="" size="10" />
                            <img src="gui/edit_pricesettings_files/en_downarrow.png" width="12" height="12" alt="en_downarrow" onclick="getOrders()"/></td>
                    ...
                    <tr>
                        <td>Doctor</td>
                        <td><input type="text" id="docName" name="docName" value="" size="30" ondblclick="getDoc()"/>
                            <input type="text" id="numRows" name="numRows"></td>
                        <td>Age</td>
    ...
    echo '</select>
                        ...
                        <td><div id="paymode" class="pay_default"></div><input type="hidden" id="pmode" name="pmode" value=""/> </td>
                    </tr>
                    <tr>
                        <td colspan="4">
    ...
    echo '<button onclick=deleteRow()>Cancel Prescription</button>';
    echo '<button onclick=printPrescriptions()>Print Prescriptions</button>';
    //  <button onclick=deleteRow()>Delete</button>
    // <button onclick=initPSearch()>Get Products List</button>';
 ...
 <tr valign=top  class="titlebar" >
  ...
  <form name="order" method="POST" action="' . $_SERVER['PHP_SELF'] . '">
                <table width=80% border=0>
                    ...
                    <tr>
                        <td>Date</td>
                        <td><input type="text" id="issDate" name="issDate" value="' . $req_date = date("Y-m-d") . '" onblur="getNextReturnNo()"/>
                            Return No:<input type="text" id="returnNo" name="returnNo" value=""/></td>
                        <td>Prescriprion No</td>
                        <td><input type="text" id="prescNO" name="prescNO" value="" size="30" 
                        ondblclick="showPrescriptions(document.order.issDate.value)" onblur="getPrescriptions(this.value)"/></td>
                    </tr>
                    <tr>
                        <td>Patient No</td>
                        <td><input type="text" id="patientId" name="patientId" value="" onblur="getPnames(this.value)" ondblclick="initPsearch()" 
                        onblur="getPrescriptions(document.order.prescNO.value)"/>
                ...
                <tr>
                        ...
                        <td><select name="storeID" id="storeID" onchange="getStore2(this.value)">
                                        <option>Select Store</option>';
    $sql = 'select st_id from care_ke_stlocation where store=1';
    $result = $db->Execute($sql);
    while ($row = $result->FetchRow()) {
        echo '<option value=' . $row[0] . '>' . $row[0] . '</option>';
    }
...
<script>
    ...
    function getStore3(str) {
        ...
        var url = "pharmVals.php?desc4=" + str;
        url = url + "&sid=" + Math.random();
    ...
    function stateChanged10()
    {
        ...
        if (xmlhttp5.readyState == 4)
        {
            var str = xmlhttp5.responseText;
            //str2=str.search(/,/)+1;
            document.order.returnNo.value = str;
        }
    }
