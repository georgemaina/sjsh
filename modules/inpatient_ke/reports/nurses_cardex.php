                ...
                echo "<div class='page'>";
                 echo "<div class='subpage'>
                        <table border=0>
                            ...
                            <tr>
                                <td colspan='6' class='summaryTitle'>CONTINUATION NURSES CARDEX</td>
                            </tr>";
                     ...
                     $sql="SELECT `date`,`time`,notes,n.`create_id`,u.`designation` FROM care_encounter_notes n 
                                LEFT JOIN care_users u ON n.`create_id`=u.`name`
                                 WHERE encounter_nr=$encNo AND u.`designation` NOT IN('DOCTOR','CO')";
                     $result=$db->Execute($sql);

                      echo "<tr><td colspan='7'><hr></td></tr>";
                     echo "<tr><td colspan='7' class='invTitle'>CONTINUATION NURSES CARDEX:</td></tr>";
					 echo "<tr><td>Date</td><td>Time</td><td colspan=4>Notes</td><td>Nurse</td>";
                     echo "<tr><td></td><td></td><td class=invDetails >";
                     while($row=$result->FetchRow()){
                         echo "<tr><td class='invDetails'>".$row['date'].' '.$row['time']."</td>
									<td class='invDetails' colspan=5>".$row['notes']."</td>
									<td class='invDetails'>".$row['create_id']."</td></tr>";                 
                     }
                       echo "<tr><td colspan='7'><hr></td></tr>";
