...
require_once ('../../../data/weberpFunctions.php');
...
switch ($caller) {
    ...
    case "getBillNumbers":
        ...
        break;
    case "finalize":
        finalizeInvoice($db, $pid, $bill_number, $startDate);
        break;
    case "getNewReceipt":
        getNewReceipt();
        break;
...
//        break;
    ...
    case "getWaitingList":
        ...
        break;
    case "assignBed":
        assignBed($pid,$encNo,$ward_nr,$room,$bed,$ward_obj);
        break;
...
function  assignBed($pid,$encNo,$ward_nr,$room,$bed,$ward_obj){
    if(($encNo=='lock')||($encNo=='unlock')){
        if($encNo=='lock') $ward_obj->closeBed($ward_nr,$room,$bed);
        else $ward_obj->openBed($ward_nr,$room,$bed);
    }else{
        if($ward_obj->AdmitInWard($encNo,$ward_nr,$room,$bed)){
            //echo "ok";
            $ward_obj->setAdmittedInWard($encNo,$ward_nr,$room,$bed);
        }
    }
...
function getWaitingList($wrdNo,$ward_obj){
    ...
    while($row=$waitlist->FetchRow()){
        ...
        echo '{"Pid":"' . $row['pid'] .'","EncounterNo":"' . $row['encounter_nr'] .'","Names":"' . $names.'","BirthDate":"' . $row['date_birth'] .'","WardNo":"'.$wrdNo.'"}';
...
function getItemsList($searchParam,$start,$limit) {
    ...
    while ($row = $result->FetchRow()) {
        ...
        . '","Category":"' . $row[item_Cat - OP] . '","Price":"' . $row[unit_price] . '","Qty":"1","Total":"' . $row[unit_price] . '"}';
...
//    var_dump($_POST);
    ...
    if (!empty($strData)) {
        updateReceiptItems($strData, $searchParam);
    } else {
        getAllReceipts($searchParam);
    }
}

function getAllReceipts($searchParam) {
    global $db;
    $debug = false;
   ...
   // $pid2 = ($pid <> "" ? $pid = $pid : $pid = '1000');
    ...
    if (isset($searchParam) && $searchParam <> '') {
        $sql = $sql . " where patient='$searchParam' OR ref_no='$searchParam'";
    }
	
	$sql=$sql." ORDER BY sale_id desc";
    ...
    while ($row = $result->FetchRow()) {
        ...
        echo '{"Sale_ID":"' . $row[sale_id] . '","Patient":"' . $row[patient] . '","CashPoint":"' . $row[cash_point] . '","ShiftNo":"' . $row[Shift_no] . '","Cashier":"' . $row[username]
        . '","ReceiptNo":"' . $row[ref_no] . '","ReceiptDate":"' . $receiptDate . '","Customer":"' . $row[name] . '","Pay_Mode":"' . $row[pay_mode]
