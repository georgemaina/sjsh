...
function AdmitInWard($ward_obj,$pid){
    $ward_nr=$_REQUEST['ward_nr'];
    $rm=$_REQUEST['rm'];
    $bd=$_REQUEST['bd'];
    if($ward_obj->AdmitInWard($pid,$ward_nr,$rm,$bd)){
        //echo "ok";
        $ward_obj->setAdmittedInWard($pid,$ward_nr,$rm,$bd);
    }
...
function getWaitingList($wrdNo,$ward_obj){
    ...
    while($row=$waitlist->FetchRow()){
        ...
        echo '{"Pid":"' . $row['pid'] .'","EncounterNo":"' . $row['encounter_nr'] .'","Names":"' . $names.'","BirthDate":"' . $row['date_birth'] .'"}';
