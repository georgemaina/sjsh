...
if($cheqID<>''){
    if(sizeof($cheqID)>1){
        $sql.="where ID IN(".substr($cheqID,0,-1).")";
    }else{
        $sql.="where ID=$cheqID";
    }
}else{
    if(sizeof($voucherNo)>1){
        $sql.="where Voucher_No IN ('$voucherNo')";
    }else{
        $sql.="where Voucher_No='$voucherNo'";
    }
...
function getVouchers($cashpoint,$voucherNo,$payMode,$page,$topPos,$leftPos) {
    ...
    $page->drawImage($image, $leftPos + 25, $topPos-100, 500, $topPos - 5);
...
WHERE cash_point='$cashpoint' AND pay_mode='$payMode' AND voucher_no='$voucherNo'";
    ...
    while ($row = $result->FetchRow()) {
        if ($topPos < 125) {
            ...
            $currpoint = 20;
//			$page->drawRectangle ( $leftPos + 32, $topPos - 5, $leftPos + 700, $topPos - 560, Zend_Pdf_Page::SHAPE_DRAW_STROKE );
        }
...
//	$page->drawRectangle ( $leftPos + 35, $topPos - 150, $leftPos + 700, $topPos - 150, Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE );
    ...
    $amtWords = convert_number($row ['Total']).$dec;
...
//    $page->drawText("Checked By:", $leftPos + 140, $topPos - 340);
    ...
    $resultsStyle->setLineDashingPattern(array(2), 1.6);
    $font = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA_BOLD);
    $resultsStyle->setFillColor(new Zend_Pdf_Color_RGB(0, 0, 0.7));
    $resultsStyle->setFont($font, 9);
    $page->setStyle($resultsStyle);
}
