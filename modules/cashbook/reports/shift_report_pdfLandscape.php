...
FROM
    ...
    $page->drawRectangle($leftPos + 9, $topPos - 95, $leftPos + 800, $topPos - 115, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
    $page->drawRectangle($leftPos + 9, $topPos - 95, $leftPos + 800, $topPos - 810, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
    ...
    $page->drawText('PartCode', $leftPos + 372, $topPos - 110);
    $page->drawText('Description', $leftPos + 440, $topPos - 110);
    $page->drawText('Pay Mode', $leftPos + 610, $topPos - 110);
    $page->drawText('Amount', $leftPos + 660, $topPos - 110);
    $page->drawText('Cheques', $leftPos + 710, $topPos - 110);
    $page->drawText('Others', $leftPos + 760, $topPos - 110);
    $currpoint = 125;
    $sql = "SELECT b.Shift_no,b.ref_no,b.`type`,b.input_time,b.patient,b.name,b.rev_code,b.rev_desc,
                ...
                a.start_time,b.currdate,b.pay_mode,b.towards
    ...
    while ($row = $result->FetchRow()) {
        if ($topPos < 150) {
            ...
            $currpoint = 20;
            $page->drawRectangle($leftPos + 9, $topPos - 5, $leftPos + 800, $topPos - 810, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
        }
        ...
        $page->drawText(trim($row['name']), $leftPos + 210, $topPos - $currpoint);
        $page->drawText($row ['rev_code'], $leftPos + 375, $topPos - $currpoint);
        if($row['rev_code']=='GL'){
            $desc=$row['towards'];
        }else{
            $desc=substr($row['prec_desc'],0,45);
        }
        $page->drawText($desc, $leftPos + 440, $topPos - $currpoint);
        $page->drawText($row ['pay_mode'], $leftPos + 620, $topPos - $currpoint);
        ...
        $pdfBase->drawText($page, number_format($cas, 2), $leftPos + 690, $topPos - $currpoint, $leftPos + 490, right);
        ...
        $pdfBase->drawText($page, number_format(intval($chq), 2), $leftPos + 740, $topPos - $currpoint, $leftPos + 530, right);
        ...
        if ($row['pay_mode'] == 'MCASH' || $row['pay_mode'] == 'mcash' || $row['pay_mode'] == '') {
            $others=$row['total'];
            $othersTotal = $othersTotal + $row['total'];
        } else {
            $others = "";
        ...
        $pdfBase->drawText($page, number_format(intval($others), 2), $leftPos + 790, $topPos - $currpoint, $leftPos + 550, right);
        ...
        $page->drawRectangle($leftPos + 10, $topPos - $currpoint - 3, $leftPos + 800, $topPos - $currpoint - 3, Zend_Pdf_Page::SHAPE_DRAW_FILL);
    }
...
//	$page->drawRectangle ( $leftPos + 35, $topPos - 150, $leftPos + 700, $topPos - 150, Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE );
    ...
    $page->drawText('Sub Totals:', $leftPos + 580, $topPos - 10);
    $pdfBase->drawText($page, number_format($casTotal, 2), $leftPos + 690, $topPos - 10, $leftPos + 690, right);
    $pdfBase->drawText($page, number_format($chqTotal, 2), $leftPos + 740, $topPos - 10, $leftPos + 740, right);
    $pdfBase->drawText($page, number_format($othersTotal, 2), $leftPos + 790, $topPos - 10, $leftPos + 790, right);
    $page->drawText('Totals:', $leftPos + 600, $topPos - 30);
    $pdfBase->drawText($page, number_format(($casTotal + $chqTotal + $othersTotal), 2), $leftPos + 690, $topPos - 30, $leftPos + 690, right);
    ...
    while ($row = $tresult->FetchRow()) {
        if ($topPos < 150) {
            ...
            $topPos = $pageHeight + 5;
            $currpoint = 5;
            $page->drawRectangle($leftPos + 32, $topPos - 5, $leftPos + 800, $topPos - 810, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
        }
...
//		$page->drawText ( $row [1], $leftPos + 200, $topPos - 95 );
        ...
        $pdfBase->drawText($page, number_format($row ['total'], 2), $leftPos + 360, $topPos - 95, $leftPos + 360, right);
    ...
    $pdfBase->drawText($page, number_format($casTotal + $chqTotal + $othersTotal, 2), $leftPos + 360, $topPos - 95, $leftPos + 360, right);
