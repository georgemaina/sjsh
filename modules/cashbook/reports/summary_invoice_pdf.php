...
function Header()
{
    ...
    $this->Cell(30,10,'SOWETO KAYOLE PHC',0,0,'L');
    //Arial bold 15
    $this->SetFont('Arial','B',15);
    //Move to the right
    //$this->Cell(120);
    ...
    $this->Cell(180,10,'INVOICE',1,1,'R');
    $this->Ln(5);
...
function LoadData()
{
    ...
    $sql="SELECT service_type,Description,price,qty,total FROM care_ke_billing WHERE pid = '$pid' order by service_type asc";
...
function getPatient()
{
    ...
    $res=mysql_query($sql);
    while($data2[]=mysql_fetch_row($res) ) { }
    return $data2;
...
function getSumAmount(){
    ...
    $sql="SELECT sum(total) as total FROM care_ke_billing WHERE pid = '$pid'";

    $res=mysql_query($sql);
    while($data3[]=mysql_fetch_row($res) ) { }
    return $data3;
}
