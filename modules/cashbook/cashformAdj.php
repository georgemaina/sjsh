...
<script language="JavaScript">
    ...
    function chkform(d) {
        if(d.cashpoint_desc.value==""){
            ...
            return false;
        }else if(d.shiftNo.value==""){
            alert("Please enter the Shift No");
            d.shiftNo.focus();
            return false;
...
<table border="0" width="80%">
    ...
    <tr>
        <td>Receipt No:</td>
        <td><input type='text' name='receiptNo' id='receiptNo' value="<?php echo $prefix.''.$receipt_no ?>" onblur="getReceiptCashpoint(this.value)"/>
            Cash Point: <input type='text' size='18' name='cash_point' id='cash_point' value="" onblur='getCashPoint(this.value)'/></td>
        <td>
            </td>
        <td valign="top">
            Shift No :
        </td>
        <td><input type='text' size='20' name='shiftNo' id='shiftNo' value=""  /></td>
        </tr>
    ...
    <tr>
        ...
        <td><input type="text" name="paymode" id="paymode" onblur="showSalesPayDesc(this.value)" value="CAS"/>
            <input type="text" size="30" name="paymode_desc" id="paymode_desc"/></td>
        <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td>G/L Acc No:</td>
    ...
    <tr>
        <td>Payer:</td>
        <td colspan="2"><input name="payer" type="text" id="payer" size="55"  /></td>
        <td></td>

        <td><input type="text" name="encounter_nr" id="encounter_nr" value="" />
            <input type="text" name="encounter_class_nr" id="encounter_class_nr" value="" /></td>
    </tr>
    <tr>
        <td>Patient:</td>
        ...
        <td> </td>
        <td>Bill Number: </td>
        <td> <input type="text" name="bill_number" id="bill_number" onblur="sumColumn(6)" onblur="getpBillNumber(document.csale.patientId.value)"/>  <input type="hidden" name="myform_key" value="<?php echo md5(date('Y:m:d H:i:s')); ?>" /></td>
        </td>
    ...
    <script>
        ...
        function getReceiptCashpoint(str){
            ...
            var url="shiftFns.php?receiptDetails="+str;
            url=url+"&sid="+Math.random();
            url=url+"&callerID=Receiptcashpoint";
            xmlhttp.onreadystatechange=stateChangedShift;
            xmlhttp.open("POST",url,true);
            xmlhttp.send(null);
        }

        function stateChangedShift()
        {
            if (xmlhttp.readyState==4)
            {
                var str=xmlhttp.responseText;
                var str2=str.split(",")
                document.csale.cash_point.value=str2[0];
                document.csale.shiftNo.value=str2[1];
            }
        }
...
//        document.csale.chequeNo.value=str2[3];
        ...
        function getpBillNumber(str,encNo)
        {
...
//            var encNo=document.getElementById('encounter_nr').value;
        ...
        function stateChangedSale()
        {
            ...
            if (xmlhttp3.readyState==4)//show point desc
            {
                ...
                //applyFilter();
                //countGridRecords(document.csale.patientId.value);
                getpBillNumber(document.csale.patientId.value,str2[3])
