...
function popDicomDownload(pid, nr, fn){
    <?php
        if($cfg['dhtml'])
        {
        echo 'w=window.parent.screen.width;
                ...
                ';
                        ...
                        ';
    ?>
    dicomwin<?php echo $sid ?>=window.open("view_dicom_download.php<?php echo URL_REDIRECT_APPEND ?>&pop_only=1&saved=1&pid=" + pid + "&img_nr=" + nr + "&fn=" + fn,"dicomwin<?php echo $sid ?>","menubar=no,resizable=yes,scrollbars=yes, width=" + (w-15) + ", height=" + (h-60) );
    <?php if($cfg['dhtml']) echo '
        window.dicomwin'.$sid.'.moveTo(0,0);'; ?>
...
<table border=0 cellpadding=0 cellspacing=0 width="100%">
  <tr>
    ...
    <td class="v12"><b>&nbsp;<?php echo 'Image Name'.$LDFileName ?></b></td>
    <td class="v12" align=center><b>&nbsp;<?php echo "Download ".$LDDownload ?></b></td>
<?php
if(!$pop_only){
?>
...
?>
  ...
  <tr>
    <td colspan=11 bgcolor="#0000ff">sdsds<img <?php echo $img_pix ?> width=1 height=1></td>
  </tr>
   <tr>
...
if($mode=='search'&&$rows){
	...
	for($i=0;$i<$z;$i++){
		if($pdata[$i]['pid']!=$pdata[($i-1)]['pid']){
			...
			$files = getFiles($sImagePath,'.jpeg');
			if($files !== FALSE){
			...
			if ($files) {
				foreach ($files as $file) {
					...
					echo '></td>
						...
						<td class="v12" align=center><a href="javascript:popDicomDownload(\''.$pdata[$i]['pid'].'\', \''.$pdata[$i]['nr'].'\', \''.$file.'\')"" title="'.$LDDownload.'"><img '.$img_download.'></a></td>';
					if(!$pop_only) echo '
