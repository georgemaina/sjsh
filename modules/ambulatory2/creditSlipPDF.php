...
$sql="SELECT b.pid,CONCAT(b.name_first,' ',name_last,' ',name_2) AS PatientNames,c.`name`,s.`Slip_no`,s.slip_date,b.date_birth FROM care_person b LEFT JOIN care_tz_company c
    ON b.`insurance_ID`=c.`id` LEFT JOIN care_ke_slips s
    ON b.`pid`=s.`pid`
    WHERE b.pid='$pid' and s.slip_date='".date('Y-m-d')."'";
//echo $sql;
...
$dob2=$dob->format('d-m-Y');

$page->drawText('AGE:  '.$dob2, $leftPos + 10, $topPos - 140);
...
//$page->drawText(strtoupper($row['PatientNames']), $leftPos + 80, $topPos - 110);
$msg= explode("\n", getWrappedText(strtoupper($row['PatientNames']), $headlineStyle, 80));  // COLUMN B
...
//$page->setStyle($normalStyle4);
 ...
 $message="Please accord medical services to ".strtoupper($row['PatientNames']) ." PID : ".$pid." whose bill is payable by ";
...
$page->drawText(strtoupper($row['name']), $leftPos + 10, $topPos - 135);
//$page->drawText('for ADMINISTRATOR', $leftPos + 10, $topPos - 140);

//$page->setStyle($normalStyle4);

$page->drawText('Printed at:', $leftPos + 10, $topPos - 170);
$page->drawText(date(' H:i:s'), $leftPos + 120, $topPos - 170);

$page->drawText('Printed By:', $leftPos + 10, $topPos - 190);
$page->drawText($inputUser, $leftPos + 120, $topPos - 190);
