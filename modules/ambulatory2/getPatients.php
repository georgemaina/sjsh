...
while ($row = $result->FetchRow()) {
    ...
    if ($rws == 1) {
        ...
        $strResult=$strResult.'<tr bgcolor="#CFCFCF" class="trColor">
            ...
            if ($dept_nr==42)
                    {
                         $temp_image="<a href=\"javascript:getARV('".$patient['pid']."','".$patient['encounter_nr']."')\">
                         <img ".createComIcon($root_path,'ball_gray.png','0','',TRUE)." alt=\"inARV\"></a>";

                    }
                    if ($dept_nr==7)
                    {
                            $temp_image="<a href=\"javascript:getEyeclinic('".$patient['pid']."','".$patient['encounter_nr']."')\">
                            <img width=17 height=17 ".createComIcon($root_path,'eye.gif','0','',TRUE)." alt=\"Eye Examination\"></a>";
                    }
