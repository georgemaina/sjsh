...
function TransferDept(dn){
   consultation=document.getElementById(dn).value;
   //alert('Consultation is '+consultation);
<?php
echo 'urlholder="amb_clinic_transfer_save.php?mode=transferdept&sid='.$sid.'&lang='.$lang.'&pyear='.$pyear.'&pmonth='.$pmonth.'&pday='.$pday.'&pn='.$pn.'&pid='.$_GET['patnr'].'&station='.$station.'&dept_nr='.$dept_nr.'&tgt_nr="+dn+"&consultation="+consultation';
?>
          // alert('Consultation is '+urlholder);
...
tr:nth-child(even) { background:#e3e8ef}
</style>
...
<table border=0>
 ...
 <table border=0 cellpadding=2 cellspacing=1 width=100%>
  <tr bgcolor="#f6f6f6">
    <td colspan=3>&nbsp;<FONT class="prompt"><?php echo $LDTransferClinic; ?></td>
  </tr>
...
function getConsultations(){
    global $db;
    $sql="Select partcode,item_description from care_tz_drugsandservices where category IN('COPD','CMCH')";
...
while(list($x,$v)=each($allmed)){
	...
	echo'</td>
         <td><select id="'.$v['nr'].'" name="'.$v['nr'].'" ><option>Consultation Fee</option>';
            echo getConsultations();
       echo '</select></td>
...
<?php $station = "'.$root_path.'modules/registration_admission/aufnahme_start.php?sid='.$sid.'&ntid=false&lang='.$lang.'&pid=$patnr&origin=patreg_reg&encounter_class_nr=1";
