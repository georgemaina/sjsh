...
$sql="SELECT c1.`batch_nr`,c1.`encounter_nr`,c1.`parameters`,c2.`item_id`,c2.`paramater_name`,
                ...
                where c1.`encounter_nr`='".$encounterNr."'";
...
while ($row = $result->FetchRow()) {
    ...
    $page->setStyle($headlineStyle);
    $page->drawText( $row['paramater_name'], $leftPos + 10, $topPos-$currPoint+10 );
    ...
    if ($row[field_type] == 'group_field') {
        $sql = "SELECT DISTINCT p.encounter_nr,k.test_date,k.test_time,p.paramater_name,p.parameter_value,
            p.job_id,p.batch_nr FROM care_test_findings_chemlabor_sub p
                ...
                WHERE p.encounter_nr='$encounterNr' AND p.paramater_name LIKE '%$row[item_id]%'
                 ORDER BY job_id ASC";
        if ($debug) echo $sql;
...
//        $currPoint2=$currPoint-90;
        $page->drawText("Name", $leftPos + 10, $topPos - 90);
        $page->drawText("Results", $leftPos + 80, $topPos - 90);
        $page->drawText("Normal", $leftPos + 110, $topPos - 90);
        $page->drawText("Ranges", $leftPos + 150, $topPos - 90);
...
//                $currpoint = 30;
            ...
            $page->drawText($row3[normal], $leftPos + 120, $topPos - $currPoint);
            $page->drawText($row3[ranges], $leftPos + 150, $topPos - $currPoint);
            $topPos = $topPos - 10;
        }
//        $topPos = $topPos - $currpoint;
        ...
        WHERE p.encounter_nr='" . $encounterNr . "'
        ...
        if ($rowCount > 0) {
            ...
            $page->drawText($row2[name] . ":", $leftPos + 12, $topPos - $currPoint+25);
            $page->drawText($row2[parameter_value], $leftPos + 90, $topPos - $currPoint+25);
        }
