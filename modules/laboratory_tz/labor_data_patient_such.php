...
/**
* CARE2X Integrated Hospital Information System Deployment 2.1 - 2017-10-02
* GNU General Public License
* Copyright 2017 George Maina
* george@chak.or.ke, 
*
...
if($search&&!empty($keyword)){
	...
	$pagen=new Paginator($pgx,$thisfile,$_SESSION['sess_searchkey'],$root_path,$oitem,$odir);
...
<B><?php echo $LDSearchWordPrompt ?></B></font>
<font size=3>
...
*/
	...
	while($zeile=$encounter->FetchRow()){
		...
		//if($zeile['encounter_nr']!=$prev_nr){
			...
			echo '</td>
					<td>';
			echo '&nbsp;'.formatDate2Local($zeile['date_birth'],$date_format,FALSE,FALSE,'-');
			echo '</td>';	
						
					echo '</td>
					<td>';
			echo '&nbsp;'.formatDate2Local($zeile['send_date'],$date_format,false,false,'-');
			echo '</td>';	
...
//                echo '<button onClick="printResults('.$zeile['pid'].','.$zeile['encounter_nr'].')"><img '.createComIcon($root_path,'update2.gif','0','absmiddle',FALSE).' alt="'.$LDClk2See.'"><font size=1> '.$LDLabReport;
