...
//echo "Encounter No is". $encounter_nr;
//echo "Local user is ".$local_user.$sid;
//if(!$local_user.$sid) {header("Location:".$root_path."language/".$lang."/lang_".$lang."_invalid-access-warning.php"); exit;}; 
...
if($encounter=&$enc_obj->getBasic4Data($_SESSION['sess_en'])) {
	...
	if ($rows=$lab_obj->LastRecordCount()){
		...
		if($nostat) header("location:".$root_path."modules/laboratory_tz/labor-nodatafound.php".URL_REDIRECT_APPEND."&user_origin=$user_origin&ln=".strtr($patient['name_last'],' ','+')."&fn=".strtr($patient['name_first'],' ','+')."&bd=".formatDate2Local($patient['date_birth'],$date_format,FALSE,FALSE,'-')."&encounter_nr=$encounter_nr&nodoc=labor&job_id=$job_id&parameterselect=$parameterselect&allow_update=$allow_update&from=$from");
...
.va12_n{font-family:verdana,arial; font-size:12px; color:#000099;font-weight: bold}
.a10_b{font-family:arial; font-size:10px; color:#000000}
.a10_n{font-family:arial; font-size:10px; color:#000099}
.a12_b{font-family:arial; font-size:12px; color:#000000}
.j{font-family:verdana; font-size:12px; color:#000000}
...
<tr valign=top><td>
            <table>
                ...
                <tr class="adm_item"><td>'.$LDBday.'</td><td class="adm-input">'.formatDate2Local($patient['date_birth'],$date_format,FALSE,FALSE,'-').'</td> </tr>
...
<tr><td>
            <table border=0 width=100% class=lab><tr>
                ...
                $sql="SELECT c1.`batch_nr`,c1.`encounter_nr`,c1.`parameters`,c2.`item_id`,c2.`paramater_name`,
                                ...
                                where c1.`encounter_nr`='$encounter_nr' order by  c1.`batch_nr` DESC";
                        if($debug) echo $sql;
                        $results=$db->Execute($sql);
                        ...
                        while($row=$results->FetchRow()){
                            ...
                            if($row['field_type']=='group_field'){
                                    ...
                                    echo "<tr><td><button id=preview onclick=getResults2($encounter_nr,$row[batch_nr],'$row[item_id]',$i)>Preview Results</button></td>
                                             <td colspan=5 id=results$i></td>
                                            <td><button id=Hide onclick=hideResults($i)>Hide Results</button></td></tr>";
                            }
