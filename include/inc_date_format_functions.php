...
if (preg_match('/inc_date_format_functions.php/',$_SERVER['PHP_SELF'])) 
...
function formatDate2Local($stdDate, $localFormat, $retTime=FALSE, $timeOnly=FALSE, $sepChars)
{
  ...
  // if(!$sepChars) 
   ...
   if(preg_match('/0000/',$stdDate))  return strtr($localFormat,'yYmMdDHis','000000000'); // IF  std date is 0 return 0's in local format
...
function formatDate2STD($localDate,$localFormat,$sepChars)
{
   ...
   //if(!$sepChars) 
	  ...
	  if(preg_match('/0000/',$finalDate)) $finalDate=0;
...
//$birthday=formatDate2STD($birthday,getDateFormat(),"-");
    $dateBirth=date_create($birthday);
    $birthday=date_format($dateBirth,"Y-m-d");
