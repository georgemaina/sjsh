...
<!--<br><br>-->
    ...
    <?php echo "<form name='labrequest' method='POST' action='".$thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&tracker=".($tracker-1)."&encounterClass=".$enclass."'>
    <br>Filter By Admission: <Select id='encounterClass' name='encounterClass' onchange=this.form.submit()>
    ...
    if($_REQUEST['encounterClass']==1){
        echo '<span style="font-weight: bold; font-size: large; color: #900000">INPATIENTS</span>';
    }elseif($enclass==2){
        echo '<span style="font-weight: bold; font-size: large; color: #900000">OUTPATIENTS</span>';
    }else{
         echo '';
    }
...
?>
<!---->
<!--<a href="--><?php //echo $thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&tracker=".($tracker-1)."&encounterClass=".$enclass; ?><!--"><img --><?php //echo createComIcon($root_path,'dwnarrowgrnlrg.gif','0','left',TRUE) ?><!-- alt="--><?php //echo $LDPrevRequest ?><!--"></a>-->
<!--<br><br>-->
<!--Pid:<input type="text" id="pid" name="pid" value='' />-->
<!--<a href="--><?php //echo $thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&pid=".$test_request['pid']."&user_origin=".$user_origin."&tracker=".($tracker-1)."&encounterClass=".$enclass; ?><!--"><img --><?php //echo createComIcon($root_path,'dwnarrowgrnlrg.gif','0','left',TRUE) ?><!-- alt="--><?php //echo $LDPrevRequest ?><!--"></a>-->
...
if($tracker<$batchrows)
{
   ...
   $test_request=$requests->FetchRow();
?>
<a href="<?php echo $thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&tracker=".($tracker+1)."&encounterClass=".$enclass; ?>"><img <?php echo createComIcon($root_path,'dwnarrowgrnlrg.gif','0','right',TRUE) ?>  alt="<?php echo $LDNextRequest ?>"></a>
<?php
}
...
while($test_request=$requests->FetchRow())
{
  ...
  {
     echo "<FONT size=2 color=\"#990000\"><b>".formatDate2Local($buf_date,$date_format,FALSE,FALSE,'-')."</b></font><br>";
	 $send_date=$buf_date;
  }
  if($batch_nr!=$test_request['batch_nr'])
  {
	...
	onmouseout=\"hideBallon(); return true;\" href=\"".$thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&tracker=".$tracker."&encounterClass=".$enclass."\">";
			if($test_request['batch_nr'])
			{
				...
				//{
					...
					echo $test_request['pid'].' '.$test_request['name_first']." ".$test_request['name_last'];
					echo '</font>';
				}
			}
