...
class Insurance_tz extends Core {
    ...
    function GetCompanyFromPID2($PID) {
        global $db;
        if (!$PID)
            return FALSE;
        $debug = false;
        $company_id = FALSE;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
        $this->sql = "SELECT c.accno FROM care_person p LEFT JOIN  care_tz_company c
                ON p.insurance_ID=c.id
                WHERE p.PID='$PID' ORDER BY id DESC LIMIT 1 ";
        $this->result = $db->Execute($this->sql);
        if($debug) echo$this->sql;
        if ($this->row = $this->result->FetchRow()) {
            $company_id = $this->row['accno'];
        }
        return $company_id;
    }
    ...
    function ConcludeContractForPID($pid, $parent, $company_id, $startup_subtraction, $plan, $companycredit) {
        ...
        if ($this->result->RecordCount() > 0) {
            ...
            echo '<font style=" font-size: xx-large; color: red"><b>No contract has been Created with this company</b></font>
                <br><font style=" font-size: large; color: blue">Go to Administrate Companies and Create a New Contract with that company</font>';
        }
            
        
        $this->result = $db->Execute($this->sql);

        $sql = 'Update care_person set insurance_ID="' . $company_id . '"';
    ...
    function setEncounterInsurance($enc_nr,$pid){
        global $db;
        $debug=true;
        
        $sql="select accno from care_tz_company where id in(select insurance_id 
        ...
        if($rcount>0){
            $row=$result->FetchRow();
            $sql="Update care_encounter set insurance_firm_id='$row[accno]' where encounter_nr='$enc_nr'";
            $db->Execute($sql);
        }
    }
...
//------------------------------------------------------------------------------
    ...
    ';
      ...
      <tr bgcolor=' . $bg . '>
      	...
      	<td align="center">' . $v['ceiling'] . ' Ksh</td>
                ...
                while (list($x, $v) = each($array)) {
                    ...
                    if (!is_array($v['Contract']) and $v['Contract'] != -2) {  //New Contract
                        ...
                        if (!$this->is_company_just_invoiced($company_id)) {
                            ...
                            if ($insurance_masterfile['credit_preselection_flag'])
                                echo 'checked'; echo '> ' . $LDGetsCredit . '</td></tr>';
                        } else {
...
//        }
    ...
    function getInsuranceID_from_accno($accno){
        global $db;
        $debug = false;
    ...
    function Get_insuranceID_from_pid($pid) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
