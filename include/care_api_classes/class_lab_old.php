...
class Lab extends Encounter {
	...
	*/
	 ...
	 * @var string
	 */
	var $tb_test_group = 'care_tz_laboratory_group';
	/**
...
($this->dead_stat)";
	...
	function TestGroups($nr = '') {
		...
		if (empty ( $nr ))
			...
			$cond .= "AND nr='$nr'";
		$this->sql = "SELECT * FROM $this->tb_test_group ORDER BY sort_nr";
...
$this->tb_req_chemlabor where batch_nr=" . $batch_nr;
...
($this->dead_stat) ORDER BY modify_time DESC";
	...
	function getGroupName($id) {
		global $db;
		$this->sql = "SELECT name FROM $this->tb_test_group WHERE group_id = '$id' AND status NOT IN ($this->dead_stat) AND nr = '$id'";
		//echo $this->sql;
		if ($this->tgroups = $db->Execute ( $this->sql )) {
			if ($this->rec_count = $this->tgroups->RecordCount ()) {
				return $this->tgroups;
