...
class Diagnostics extends Encounter {
  ...
  var $tbl_diagnosis_dead="pataintStatus";
  ...
  function GetAllCasesFromPIDbyDate($pid) {
		...
		$this->sql="SELECT * FROM ".$this->tbl_diagnosis." WHERE pid='".$pid."' ORDER BY Timestamp DESC";
		$d_rs = $db->Execute($this->sql);
		while ($d_row=$d_rs->FetchRow()) {
  ...
  //-----------------------------------------------------------------------------------------------------------------
    ...
    function getCurrentDepartment($encNr){
        global $db;
        $debug=false;

        $sqll="select current_dept_nr from care_encounter where encounter_nr='$encNr'";
        $results=$db->Execute($sqll);
        $row=$results->FetchRow();
        $deptNo=$row['current_dept_nr'];
    ...
    function checkCurrentEncounterNotes($encNr){
        ...
        $this->sql="SELECT COUNT(n.encounter_nr) AS diagnosis FROM `care_encounter_notes` n
                ...
                WHERE n.encounter_nr='$encNr' AND e.`encounter_class_nr`=2";
        if($debug) echo $this->sql;
        $results=$db->Execute($this->sql);
    ...
    function checkCurrentEncounterVitals($encNr){
        ...
        $this->sql="SELECT COUNT(m.encounter_nr) AS vitals FROM `care_encounter_measurement` m 
                ...
                WHERE `msr_date`>'2018-01-01' AND m.encounter_nr='$encNr'";
        if($debug) echo $this->sql;
        $results=$db->Execute($this->sql);
     ...
     function checkCurrentBPStatus($encNr){
        global $db;
        $debug=false;
  ...
  function get_icd10_description_from_array($rs) {
    ...
    while ($row=$rs->FetchRow()) {
      ...
      $this->sql="SELECT diagnosis_code, description FROM ".$this->tbl_icd10." WHERE diagnosis_code='".$row['diagnosis_code']."' ORDER BY diagnosis_code asc";
      $d_rs = $db->Execute($this->sql);
      //return $d_rs;
  ...
  function get_icd10_description_from_code($code) {
    ...
    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
      ...
      if ($debug) echo $code;
      $this->sql="SELECT diagnosis_code, description FROM ".$this->tbl." WHERE diagnosis_code='".$code."' ORDER BY diagnosis_code asc";
      $d_rs = $db->Execute($this->sql);
      //return $d_rs; O
      //echo $this->sql;
    ...
    while(list($x,$v) = each($dataarray))
    {
    	if(strstr($x,"diagnosistype"))
    	{
    		...
    		if($dataarray['diagnosistype_'.$form_id]=="revisit")
    			$return = $this->insert_diagnosis_as_revisit($dataarray['parentcase_'.$form_id], 
                                $dataarray['pid'], $dataarray['encounter'], $dataarray['icd_'.$form_id],
                                $dataarray['icddesc_'.$form_id], $dataarray['comment_'.$form_id], 
                                $dataarray['diagnosistype_'.$form_id], $dataarray['doctor_'.$form_id],$dead);
    		else
    			$return = $this->insert_diagnosis_as_new($dataarray['pid'], $dataarray['encounter'], 
                                $dataarray['icd_'.$form_id], $dataarray['icddesc_'.$form_id], 
                                $dataarray['comment_'.$form_id], $dataarray['diagnosistype_'.$form_id], 
                                $dataarray['doctor_'.$form_id],$dead);
  ...
  function insert_diagnosis_as_new($pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, 
          $type, $doctor,$dead) {
    global $db;
    $debug=false;
    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
  ...
  function insert_diagnosis_as_revisit($parent_case_nr, $pid, $encounter_nr, $ICD10_code, $ICD10_description,
          $comment, $type, $doctor,$dead) {
    global $db;
    $debug=false;
    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
  ...
  */
  ...
  function Display_Archived_Diagnoses($pid){
    ...
    echo '<form action="" method="post">
      ...
      $idcounter=0;
    foreach ($this->icd_old_array as $x=>$v){
    	$idcounter++;
