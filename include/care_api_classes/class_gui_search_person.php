	...
	*/
	...
	function display($skey=''){
		...
		if ($linecount){
			...
			$this->smarty->assign('LDName3',$pagen->makeSortLink($LDName3,'name_2',$oitem,$odir,$this->targetappend));
            $this->smarty->assign('LDZipCode',$pagen->makeSortLink($LDFileNr,'selian_pid',$oitem,$odir,$this->targetappend));
            $this->smarty->assign('LDVillage',$pagen->makeSortLink($LDVillage,'citizenship',$oitem,$odir,$this->targetappend));
            $this->smarty->assign('LDPhone',$pagen->makeSortLink($LDPhone,'phone_1_nr',$oitem,$odir,$this->targetappend));
			...
			if(!empty($this->targetfile)){
				$this->smarty->assign('LDOptions',$LDOptions);
                         ...
                         //$this->smarty->assign('LDTownCity',$pagen->makeSortLink($LDTownCity,'TownCity',$oitem,$odir,$this->targetappend));
			...
			while($zeile=$ergebnis->FetchRow()){
				...
				if($zeile['status']=='' || $zeile['status']=='normal'){
					...
					$this->smarty->assign('sFirstName',ucfirst($zeile['name_first']));
                                        $this->smarty->assign('sName3',ucfirst($zeile['name_2']));
					#
                    ...
                    $this->smarty->assign('sVillage',ucfirst($zeile['citizenship']));
                    $this->smarty->assign('sPhone',ucfirst($zeile['phone_1_nr']));

					$this->smarty->assign('sBday',formatDate2Local($zeile['date_birth'],$date_format,$retTime=FALSE, $timeOnly=FALSE, "/"));
