...
class Encounter extends Notes {
    ...
    /**
     *Table name for Financial plan types
     * @var string
     */
    var $tb_pricetypes='care_ke_priceTypes';
    ...
    function loadEncounterData($enc_nr = '') {
        ...
        if (!$this->internResolveEncounterNr($enc_nr))
            return FALSE;
        $this->sql = "SELECT e.*,e.`financial_class`, p.pid, p.selian_pid, p.title,p.name_last, p.name_first, p.date_birth, p.sex,
									p.addr_str,p.addr_str_nr,p.addr_zip, p.blood_group,p.name_maiden,
									p.photo_filename, t.name AS citytown_name,p.death_date,c.pricetype
							FROM $this->tb_enc AS e 
                            LEFT JOIN care_ke_pricetypes AS c ON e.`financial_class`=c.`id`,
									 $this->tb_person AS p
    ...
    function Sex2($enr = 0) {
        global $db;
        $debug = false;
        $this->sql = "SELECT c.sex FROM care_person c left join care_encounter b on c.pid=b.pid where b.encounter_nr= $enr";
        if ($debug)
            echo $this->sql;
    ...
    function EncounterTime($enr = 0) {
        if ($this->is_loaded) {
            return $this->encounter['encounter_time'];
        } else {
            if ($enr)
                return $this->getValue('encounter_time', $enr);
            else
                return FALSE;
        }
    }
    ...
    function getFinanceClassByName($nr){
     global $db;

     $sql="select priceType from care_ke_pricetypes where ID='$nr'";
    ...
    function isCurrentlyAdmitted($nr, $type = '_ENC') {
        ...
        $this->sql = "SELECT encounter_nr FROM $this->tb_enc
						WHERE $cond='$nr' AND encounter_status <> 'cancelled' and encounter_class_nr=1 AND (is_discharged='' OR is_discharged=0) AND status NOT IN ($this->dead_stat)";
    ...
    function getAdmissionDetails($enc_nr){
        global $db;
        $debug=false;

        $sql="SELECT SELECT pid,encounter_nr,encounter_date,e.`current_ward_nr`,w.`ward_id`,`current_room_nr` FROM care_encounter e
                    LEFT JOIN care_ward w ON e.`current_ward_nr`=w.`nr` WHERE encounter_nr=$enc_nr";
    ...
    function getOPEncounterDetails($enc_nr){
        global $db;
        $debug=false;

        $sql="SELECT e.encounter_nr,e.encounter_date,e.current_dept_nr,d.`name_formal`,l.`date_from`,l.`time_from`,l.`date_to`,l.`time_to`
    ...
    function getCurrentEncounterNr($pid){
        global $db;
        $debug=false;
        $sql="SELECT MAX(encounter_nr) AS encounter_nr FROM care_encounter WHERE pid='$pid'";
        if($debug) echo $sql;
    ...
    function getCurrentEncounter($pid){
        ...
        $sql = "SELECT b.`pid`,b.name_first,b.name_2,b.name_last,MAX(a.encounter_nr) AS encounter_nr,a.encounter_class_nr,
                    a.current_ward_nr,w.description as ward,a.`bill_number`,a.`encounter_date`,a.`discharge_date`,
    ...
    function getEncounterDetails($enc_nr) {
        global $db;
        $debug=false;
        $this->sql = "SELECT * FROM care_encounter WHERE encounter_nr=$enc_nr";
        if($debug) echo $this->sql;
    ...
    function _searchAdmissionBasicInfo($key, $enc_class = 0, $add_opt = '', $limit = FALSE, $len = 30, $so = 0) {
        ...
        if (is_numeric($key)) {
            ...
            $key = (int) $key;
            if ($enc_class == 2) {
                $this->sql.=" WHERE  p.pid = $key " . $add_opt;
            } else {
                $this->sql.=" WHERE p.pid = $key AND  e.is_discharged IN ('',0)" . $add_opt;
            }
        } elseif ($key == '%' || $key == '*') {
            if ($db->debug)
    ...
    /**
     * Saves the encounters ward location.
    ...
    function _setCurrentAssignment($enr, $data = '', $act = 'Modified') {
         ...
         */
        $outpatient=$this->EncounterClass($enr);

        $data.=",history=" . $this->ConcatHistory("\n$act " . date('Y-m- H:i:s') . " " . $_SESSION['sess_user_name']) . ", ";
        $data.="	modify_id='" . $_SESSION['sess_user_name'] . "',
				modify_time='" . date('YmdHis') . "', in_dept=1 , is_discharged=0 ";

        if ($outpatient == 2) {
            //case transfer from outpatient-department to ward
            $data.=", encounter_class_nr=$outpatient";
        }
...
//		global $HTTP_SESSION_VARS;
     ...
     * @return boolean
     */
    function setIsDischarged($enr, $x_date, $time) {
        ...
        $this->sql = "UPDATE $this->tb_enc SET is_discharged=1, discharge_date='$x_date',discharge_time='$time',release_date='$x_date', in_ward=0,in_dept=0, status='discharged', history =" . $this->ConcatHistory("Update (discharged): " . date('Y-m-d H:i:s') . " " . $_SESSION['sess_user_name'] . "\n") . " WHERE encounter_nr=$enr AND is_discharged IN ('',0,'9')";
    ...
    function getDischargeTypesData() {
        global $db;
        $db->debug = 0;
        $this->sql = "SELECT nr,name,LD_var AS \"LD_var\" FROM $this->tb_dis_type WHERE `status`<>'disabled' ORDER BY nr";
        //echo $this->sql;
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->result->RecordCount()) {
                return $this->result;
    ...
    function checkDischargeType($encounterNr)
    {
        global $db;
        $debug = false;
...
//
     ...
     * @return boolean
     */
    function Discharge($enr, $d_type_nr, $x_date, $time ) {
        if ($this->_discharge($enr, "'1','2','3','4','5','6'", $d_type_nr, $x_date, $time,FALSE)) {
            if ($this->setIsDischarged($enr, $x_date, $time)) {
     ...
     * @return boolean
     */
    function DischargeMedically($enr, $d_type_nr, $date = '', $time = '') {
//        if ($this->_discharge($enr, "'2','4','5'", $d_type_nr, $date, $time)) {
//            return true;
//        } else {
//            return FALSE;
//        }
        $x_date=date('Y-m-d');
        $time=date('H:i:s');
        $this->sql = "UPDATE $this->tb_enc SET is_discharged=9, discharge_date='$x_date',discharge_time='$time', in_ward=0,in_dept=0, status='discharged', history =" . $this->ConcatHistory("Update (discharged): " . date('Y-m-d H:i:s') . " " . $_SESSION['sess_user_name'] . "\n") . " WHERE encounter_nr=$enr AND is_discharged IN ('',0)";
    ...
    function getBasic4Data($enc_nr) {
        ...
        if (!$this->internResolveEncounterNr($enc_nr))
            return FALSE;
        $this->sql = "SELECT p.name_last, p.name_first, p.date_birth, p.sex,p.name_2,p.pid,p.selian_pid
							FROM $this->tb_enc AS e,
									 $this->tb_person AS p
							WHERE e.encounter_nr=$this->enc_nr
								AND e.pid=p.pid";
