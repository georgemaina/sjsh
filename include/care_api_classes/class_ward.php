...
class Ward extends Encounter {
	...
	* @access private
     ...
     * @return boolean
     */
    function _getBedCharge($room_nr,$ward_nr) {
        global $db, $dbf_nodate;
        $db->debug=0;
        $this->sql='SELECT charge from care_room
                    where room_nr="'.$room_nr.'" and ward_nr="'.$ward_nr.'"';
        if($this->result=$db->Execute($this->sql)) {
    ...
    function _getWardOccupants($ward_nr,$date) {
       ...
       $this->sql="SELECT p.pid,p.name_first,p.name_last,p.`name_2`,p.`date_birth`,p.sex,p.title,p.photo_filename,
                            e.`current_ward_nr` as wardID,e.`current_room_nr` as room_nr,l.`type_nr`,l.`group_nr`,
                            l.location_nr as bed_nr ,e.encounter_date,c.name as insurance,e.encounter_nr,e.bill_number
                        FROM care_person p
    ...
    function _getWardOccupants2($ward_nr,$date) {
        global $db, $dbf_nodate;
        $db->debug=0;
        ...
        $this->sql="SELECT r.location_nr AS room_nr,
                        ...
                        n.nr AS ward_notes
        FROM $this->tb_location AS r tb_ic
    ...
    function checkIfCharged() {
        global $db;
        $db->debug=0;
        $this->sql='select pid from care_ke_billing where partcode="bedc" and bill_date="'.date("Y-m-d").'"';
    ...
    function getAllAdmitedPatientsInfo() {
        global $db;
        $db->debug=0;
        $this->sql='SELECT a.encounter_nr,a.pid, b.name_first,b.name_2,	b.name_last,a.encounter_class_nr,
	a.encounter_status,a.current_ward_nr, a.current_room_nr,a.in_ward,a.is_discharged
        FROM care_encounter a, care_person b WHERE a.pid=b.pid AND a.encounter_class_nr=1  AND a.is_discharged =0';
        ...
        function updateBedCharge($enc_nr,$pid,$billnumber){
        global $db;
        $this->debug=false;
//        $bill_number='111111';


        $row2=$this->EncounterLocationsInfo($enc_nr);
        $bed_nr=$row2[6];
        $room_nr=$row2[5];
        $ward_nr=$row2[0];
        $ward_name=$row2[1];
        $charge=$this->_getBedCharge($room_nr, $ward_nr);
        if ($this->debug) echo $charge;

        $sql3 = "INSERT INTO care_ke_billing (pid, encounter_nr,bill_date,`ip-op`,bill_number,
                            service_type, price,`Description`,prescribe_date,
                            input_user,item_number,partcode,status,qty,total,ward_nr,ward_name,room_nr,bed_nr)
                            value('".$pid."','".$enc_nr."','".date("y-m-d")."',1,'"
                .$billnumber."','bed_charge','".$charge."','Bed Charge','".date("y-m-d")
                ."','Admin','bedc','bedc','insured'"
                .",'1','".$charge."','".$ward_nr."','".$ward_name."','".$room_nr."','".$bed_nr."')";

        $db->Execute($sql3);
//        if ($this->debug) echo $sql3."<br>";
        }
    ...
    //Update ward charge for the room
     function updateRoomCharge($data) {
        global $db;
         $debug=false;
        $this->sql="UPDATE care_room SET charge='".$data['charge']."' WHERE nr=".$data['nr'];
         if($debug) echo $this->sql;
        $db->Execute($this->sql);
        return true;
    }

    //Update Nursing fee for the room
    function updateRoomCharge2($data) {
        global $db;
        $debug=false;
        $this->sql="UPDATE care_room SET charge2='".$data['charge2']."' WHERE nr=".$data['nr'];
        if($debug) echo $this->sql;
        $db->Execute($this->sql);
        return true;
    }
    ...
    //Update Doctors Fee
    function updateRoomCharge3($data) {
        global $db;
        $debug=false;
        $this->sql="UPDATE care_room SET charge3='".$data['charge3']."' WHERE nr=".$data['nr'];
        if($debug) echo $this->sql;
        $db->Execute($this->sql);
        return true;
    }

    //Update Accommodation charges
    function updateRoomCharge4($data) {
        global $db;
        $debug=false;
        $this->sql="UPDATE care_room SET charge4='".$data['charge4']."' WHERE nr=".$data['nr'];
        if($debug) echo $this->sql;
    ...
    //Update number of Beds in the room
    ...
    //Update first Bed Number in that room
    function updateFirstBedno($data) {
        global $db;
        $debug=false;
        $this->sql="UPDATE care_room SET firstBedNo='".$data['firstbedno']."' WHERE nr=".$data['nr'];
        if($debug) echo $this->sql;
        $db->Execute($this->sql);
        return true;
    }

    //Update last Bed Number in that room
    function updateLastBedNo($data) {
        global $db;
        $debug=false;
        $this->sql="UPDATE care_room SET lastBedNo='".$data['lastbedno']."' WHERE nr=".$data['nr'];
        if($debug) echo $this->sql;
        $db->Execute($this->sql);
        return true;
    }
    ...
    function EncounterLocationsInfo($enc_nr) {
        ...
        $this->sql="SELECT w.ward_id,w.name AS ward_name, w.roomprefix,
				...
				FROM $this->tb_enc AS e
					...
					OR	(e.encounter_class_nr=1 AND e.current_dept_nr=d.nr)
      ...
      function EncounterLocationsInfo2($enc_nr) {
        global $db;
        $db->debug=0;
        $this->sql="SELECT w.ward_id,w.name AS ward_name, w.roomprefix,
        d.id AS dept_id,d.name_formal AS dept_name,
        ...
        LEFT JOIN $this->tb_dept AS d ON (e.encounter_class_nr=1 AND e.current_ward_nr=d.nr)
        OR	(e.encounter_class_nr=1 AND e.current_dept_nr=d.nr)
        LEFT JOIN $this->tb_location AS r ON r.encounter_nr=$enc_nr AND r.group_nr=w.nr AND r.type_nr=4
        LEFT JOIN $this->tb_location AS b ON b.encounter_nr=$enc_nr AND  b.group_nr=w.nr AND b.type_nr=5
        WHERE e.encounter_nr=$enc_nr AND e.status NOT IN ($this->dead_stat)";
