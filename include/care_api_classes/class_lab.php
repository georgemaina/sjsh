...
class Lab extends Encounter {
	...
	*/
     ...
     *
     */
    var $tb_test_results='care_tz_laboratory_resultstypes';
    /**
	...
	function createRequestsList($enc_nr) {
        ...
        function GetJobID($enc_nr){
            global $db;
            
            $sql="SELECT max(batch_nr) as batch_nr FROM care_test_request_chemlabor where encounter_nr='$enc_nr'";
...
($this->dead_stat)";
	...
	function getResult($job_id, $grp_id, $enc_nr = '') {
		...
		$this->sql = "SELECT * FROM $this->tb_find_chemlab INNER JOIN $this->tb_find_chemlab_sub ";
		$this->sql .= "ON ($this->tb_find_chemlab.job_id = $this->tb_find_chemlab_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_chemlab.encounter_nr='$this->enc_nr' 
                                AND $this->tb_find_chemlab.job_id='$job_id' AND $this->tb_find_chemlab.group_id is null AND $this->tb_find_chemlab.status<>'hidden'";
	...
	function getAllResults($enc_nr = '') {
		...
		$this->sql = "SELECT * FROM $this->tb_find_chemlab INNER JOIN $this->tb_find_chemlab_sub ";
		$this->sql .= "ON ($this->tb_find_chemlab.job_id = $this->tb_find_chemlab_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_chemlab.encounter_nr='$this->enc_nr' ORDER BY $this->tb_find_chemlab_sub.test_date";
	...
	function GetTestsToDo($id = '') {
		...
		if (empty ( $id ))
			...
			$cond = "batch_nr='$id'";
		$sub = "_sub";
		$this->sql = "SELECT paramater_name, parameter_value,item_id FROM $this->tb_req_chemlab$sub  WHERE $cond";
		//echo $this->sql;
		if ($this->tparams = $db->Execute ( $this->sql )) {
			if ($this->rec_count = $this->tparams->RecordCount ()) {
				return $this->tparams;
		...
		if (empty ( $id ))
			...
			$cond = "batch_nr='$id'";
		$sub = "_sub";
		$this->sql = "SELECT s.paramater_name, s.parameter_value,s.item_id,p.`name` as testName,p.field_type,p.add_type,p.id
                              FROM care_test_request_chemlabor_sub s
                                LEFT JOIN `care_tz_laboratory_param` p ON s.`paramater_name`=p.`id`
                                WHERE $cond";
              //  echo $this->sql;
		if ($this->tparams = $db->Execute ( $this->sql )) {
			if ($this->rec_count = $this->tparams->RecordCount ()) {
        ...
        function GetResultParams($id = '') {
		global $db;
		if (empty ( $id ))
			$cond = '';
		else
			$cond = " WHERE r.item_id='$id'";
                
		$this->sql = "SELECT r.resultID,r.item_id,r.results,r.input_type,r.normal,r.ranges,r.result_values,p.name FROM `care_tz_laboratory_resultstypes` r
                        LEFT JOIN care_tz_laboratory_param p ON r.item_id=p.item_id $cond";
//        echo $this->sql;
	...
	function TestParamsDetails($id = '') {
		...
		if (empty ( $id ))
			...
			$this->sql .= "LEFT JOIN $this->tb_test_param_type ON ( $this->tb_test_param.id = $this->tb_test_param_type.name ) ";
			$this->sql .=" $cond ";
			//echo 'SQL: '.$this->sql;
    ...
    /**
     * Returns all test results parameters.
     * george maina : Created to get all results parameters, values and input methods
     * @access public
     * @return mixed adodb record object or boolean
     */
    function TestResultsParams($nr = '') {
        global $db;
        ...
        if (empty ( $nr ))
            ...
            $cond .= "AND nr='$nr'";
        $this->sql = "SELECT * FROM $this->tb_test_results $cond ORDER BY sort_nr";
...
the database
	...
	function IsMissingLabResult($batch_nr) {
		...
		$param_array = array ();
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		$this->sql = "SELECT DISTINCT parameters FROM care2x.care_test_request_chemlabor";//where batch_nr='" .$batch_nr."'";
		$this->request = $db->Execute ( $this->sql );
		$row = $this->request->GetArray ();
...
($this->dead_stat) ORDER BY modify_time DESC";
	...
	function TestActiveGroups() {
		...
		if($debug) echo $this->sql;
		//$this->sql = "SELECT * FROM $this->tb_test_param ORDER BY sort_nr";
//                echo $this->sql;
		if ($this->tgroups = $db->Execute ( $this->sql )) {
			if ($this->rec_count = $this->tgroups->RecordCount ()) {
				return $this->tgroups;
