...
<?php
		...
		<?php global $db;
		    	...
		    	$sql="SELECT name FROM care_tz_company WHERE ID=$insurance_ID";
//                        echo $coreObj->sql;
		...
		<td><?php echo $nat_id_nr; ?></td>
		</tr>
                <tr>
		<td>
			<FONT SIZE=-1  FACE="Arial">
			<?php echo $LDMemberNumber; ?>:
		</td>
		...
		<td><?php echo $next_of_kin .' :'.$LDkinrelation .'  <font SIZE=2  FACE="Arial" color="#990000" weight="bold">'.$kin_relations  ?></td></font></td>
		</tr>
		...
		<td><?php echo $next_of_kin2.' :'.$LDkinrelation .' <font SIZE=2  FACE="Arial" color="#990000" >'.$kin_relations2   ?></td>font></td></td>
		</tr>
 ...
 <?php
		...
		if (!$GLOBAL_CONFIG['person_religion_hide']&&$religion){
			$this->createTR($LDReligion,$name,2);
                ...
                $sql1="SELECT c.name FROM care_tz_company c LEFT JOIN care_person d ON c.id=d.insurance_id WHERE pid=$pid";
				if ($results = $db->Execute($sql1))
				{
					$row = $results->FetchRow();
					$insurance_name1 = $row['name'];
				}
		if (!$GLOBAL_CONFIG['person_insurance_hide']&&$insurance_ID){
			$this->createTR($LDInsurance,$insurance_name1,2);
		}
