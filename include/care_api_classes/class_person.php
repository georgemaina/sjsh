...
class Person extends Core {
	...
	*/
	var $basic_list='pid,title,name_first,name_last,name_2,name_3,name_middle,name_maiden,name_others,date_birth,
				           sex,addr_str,addr_str_nr,addr_zip,addr_citytown_nr,citizenship,photo_filename,
                                           email,sss_nr,nat_id_nr,region,district,county,ward,next_of_kin,kin_relations,
                                           next_of_kin2,kin_relations2,SourceClinic,memberNumber';
	/**
	...
	*/
	var  $elems_array=array(
				...
				'photo',
				 ...
				 'create_time',
				 'addr_citytown_name',
				 'insurance_ID','next_of_kin','next_of_kin2','kin_relations','kin_relations2','SourceClinic','memberNumber');
	/**
	* Constructor
	...
	*/
	function SelianFileExists($selian_pid){
        ...
        function DentalFileExists($selian_pid){
		global $db;
		// Patch for db where the pid does not start with the predefined init
		//$this->sql="SELECT pid FROM $this->tb_person WHERE pid=$init_nr";
		$this->sql="SELECT selian_pid FROM $this->tb_person WHERE selian_pid=$selian_pid";
	...
	function GetNewSelianFileNumber(){
		...
		// Patch for db where the pid does not start with the predefined init
		//$this->sql="SELECT pid FROM $this->tb_person WHERE pid=$init_nr";
		$this->sql="SELECT max(selian_pid) as maximum FROM $this->tb_person WHERE selian_pid NOT LIKE 'DU%'";
		if($this->result=$db->Execute($this->sql)){
...
#        global $HTTP_POST_VARS;
		...
		foreach ($this->elems_array as $x => $v) {
	...
	*/
    function insertDataFromArray($array) {
		...
		$pid_counter_min = PID_PREFIX*1+1;
	...
	*/
	function insertDataFromInternalArray() {
		...
		if(!isset($this->buffer_array['create_time'])||empty($this->buffer_array['create_time'])) $this->buffer_array['create_time']=date('YmdHis');
	      if($this->insertDataFromArray($this->data_array)){
                  return true;
              }else{
                  echo $db->Error;
              }
...
*/
	...
	*/
	function getAllInfoObject($pid='') {
	    ...
	    $this->sql="SELECT p.*, citizenship AS addr_citytown_name,ethnic.name AS ethnic_orig_txt, tribe.tribe_name , religion.name
                            ,region.region_name  ,district.district_name ,ward.ward_name
	...
	*/
    ...
    function memberNumber() {
        return  $this->getValue('memberNumber');
    }
	...
	*/
	...
	/**
	* Returns payrollStidID.
	*/
	function payrollStidID() {
        return $this->getValue('payrollStidID');
	}
	/**
	/**
	...
	*/
	function DeathCause() {
        ...
        function SourceClinic() {
        return  $this->getValue('SourceClinic');
	}
	/**
	...
	*/
	function CurrentEncounter($pid){
	    global $db;
		if(!$pid) return false;
		$sql="SELECT encounter_nr,encounter_class_nr,current_dept_nr,financial_class,consultation_fee,current_ward_nr,encounter_date,encounter_time FROM $this->tb_enc WHERE pid='$pid' AND (is_discharged='' OR is_discharged=0) AND 
                    encounter_status <> 'cancelled' AND status NOT IN ($this->dead_stat)";
//		echo $sql;
        ...
        function CurrentMaxEncounter($pid){
	    global $db;
		if(!$pid) return false;
		$sql="SELECT max(encounter_nr) as encounter_nr FROM $this->tb_enc WHERE pid='$pid' AND encounter_status <> 'cancelled' AND status NOT IN ($this->dead_stat)";
//		echo $sql;
                if($buf=$db->Execute($sql)){
                    if($row=$buf->FetchRow()){
...
*/
	...
	*/
	...
	function SearchSelect($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE){
		...
		if(substr($suchwort,-3,1)=='/'){
                        $sql2="	WHERE selian_pid='$suchwort'";
                

		}else if(is_numeric($suchwort)) {
...
//			$this->is_nr=TRUE;
			...
			$sql2="	WHERE pid=$suchwort or selian_pid='$suchwort' or nat_id_nr='$suchwort' or citizenship = '$suchwort' or phone_1_nr='$suchwort'";
		} else {
			...
			# Arrange the values, ln= lastname, fn=first name, bd = birthday,sn=surname
			if($lastnamefirst){
				$fn=$comp[1];
				$ln=$comp[0];
				$bd=$comp[3];
			...
			if(sizeof($comp)==2){
				...
				$sql2=" WHERE (name_first $sql_LIKE '".strtr($fn,'+',' ')."%' AND name_2 $sql_LIKE '".strtr($ln,'+',' ')."%')";
				if(!empty($bd)){
             ...
             }else if(sizeof($comp)>2){
                $sql2=" WHERE (name_first $sql_LIKE '".strtr($fn,'+',' ')."%' AND name_last $sql_LIKE '".strtr($sn,'+',' ')."%' AND name_2 $sql_LIKE '".strtr($ln,'+',' ')."%')";
//				if(!empty($sn)){
//				    $sql2.=" AND name_2 $sql_LIKE '$sn%' ";
//				}
			}else{
				# Check if * or %
				if($suchwort=='%'||$suchwort=='%%'){
					...
					if($DOB=='') {
						if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
							if($fname){
								$sql2=" WHERE name_last $sql_LIKE '".strtr($suchwort,'+',' ')."%' OR 
                                                                        name_first $sql_LIKE '".strtr($suchwort,'+',' ')."%'";
							}else{
					...
					$sql2.=" AND status NOT IN ($this->dead_stat) or citizenship = '$suchwort' or phone_1_nr='$suchwort'";
				}
			}
		 }
		...
		//$this->buffer=$frResetPasswordomwhere;
		//$sql2.=' AND status NOT IN ("void","hidden","deleted","inactive")  ORDER BY '.$oitem.' '.$odir;
		# Set the sorresetting directive
		if(isset($oitem)&&!empty($oitem)) $sql3 =" ORDER BY $oitem $odir";

		$this->sql='SELECT pid, selian_pid, name_last, name_first,name_2, date_birth, addr_zip, sex, death_date, status,citizenship,phone_1_nr FROM '.$this->buffer.$sql3;
//		echo $this->sql;
	...
	*/
	function PIDbyData($data,$deadtoo=FALSE){
		...
		$this->sql="SELECT pid,name_last,name_first,date_birth,sex,citizenship,phone_1_nr FROM $this->tb_person WHERE name_last $sql_LIKE '".$data['name_last']."'
					AND name_first $sql_LIKE '".$data['name_first']."'
					AND date_birth='".$data['date_birth']."'
					AND citizenship='".$data['citizenship']."'
