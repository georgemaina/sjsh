...
dhtmlXWindows.prototype._dhx_Engine = function() {
	...
	// th = toolbar height
	// sh = statusbar height
	this._engineSkinParams = {"dhx_blue":	{"hh": 21, "lbw": 2, "rbw": 2, "lch": 2, "lcw": 14, "rch": 14, "rcw": 14, "bbh": 2, "mnh": 23, "tbh": 25, "sbh": 20},
				  "dhx_black":	{"hh": 21, "lbw": 2, "rbw": 2, "lch": 2, "lcw": 14, "rch": 14, "rcw": 14, "bbh": 2, "mnh": 23, "tbh": 25, "sbh": 20},
				  "dhx_skyblue":{"hh": 29, "lbw": 2, "rbw": 2, "lch": 2, "lcw": 14, "rch": 14, "rcw": 14, "bbh": 2, "mnh": 23, "tbh": 25, "sbh": 20}
	};
	...
	this._engineRedrawWindowSize = function(win) {
		...
		/*
		var data = body.childNodes[(_isIE?5:4)];
		
		// console.log(data==win.dhxcont)
		
		data.style.top = hh+"px";
		data.style.left = "0px";
		data.style.width = body.clientWidth+"px";
		// park check
		var px = body.clientHeight-hh;
		if (px < 0) { px = 0; }
		//
		data.style.height = px+"px";
		
		if (data.offsetWidth > body.clientWidth) { data.style.width = body.clientWidth*2-data.offsetWidth+"px"; }
		if (data.offsetHeight > body.clientHeight - hh) {
			// park check
			var px = (body.clientHeight-hh)*2-data.offsetHeight;
			if (px < 0) { px = 0; }
			//
			data.style.height = px+"px";
		}
		...
		var cont = this._engineGetWindowContent(win);//data.childNodes[0];
		cont.style.width = data.clientWidth+"px";
		cont.style.height = data.clientHeight-(win.menu!=null?win.menuHeight:0)-(win.toolbar!=null?win.toolbarHeight:0)-(win.sb!=null?win.sbHeight:0)+"px";
		*/
	}
