...
if (preg_match('/inc_passcheck.php/',$_SERVER['PHP_SELF'])) 
...
function validarea($zeile2, $permit_type_all = 1){
    global $allowedarea;
//    echo var_dump($allowedarea)."<br><br><br>";
//    echo $zeile2;
//    echo sizeof($allowedarea);
...
($debug)?$db->debug=TRUE:$db->debug=FALSE;
    ...
    $sql="SELECT name, login_id, password, permission, lockflag FROM care_users WHERE login_id='$userid'";
	...
	if($ergebnis=$db->Execute($sql))
	{
		...
		if (($zeile['password']==md5($keyword))&&($zeile['login_id']==$userid)) 
		{
			...
			if (!($zeile['lockflag']))
			{
               // echo 'Am Here and check '.$screenall;
