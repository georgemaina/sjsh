...
function update_db()//called only when _viewing_ scheduled tasks
{
 ...
 $result=pjs_mysql_query("select scriptpath from ".PJS_TABLE);
 if (mysql_field_len($result, 0)<255) $result=pjs_mysql_query("ALTER TABLE ".PJS_TABLE." CHANGE scriptpath scriptpath VARCHAR(255)");
 $result=pjs_mysql_query("SHOW COLUMNS FROM ".PJS_TABLE." LIKE 'run_only_once' ");
 if (!mysql_num_rows($result)) $result=pjs_mysql_query("ALTER TABLE ".PJS_TABLE." ADD run_only_once tinyint(1) NOT NULL DEFAULT '0'");
 $result=pjs_mysql_query("SHOW COLUMNS FROM ".PJS_TABLE." LIKE 'currently_running' ");
 if (!mysql_num_rows($result)) $result=pjs_mysql_query("ALTER TABLE ".PJS_TABLE." ADD `currently_running` BOOLEAN NOT NULL DEFAULT '0' AFTER `run_only_once`");
}
...
function save_log($script,$output,$execution_time)
{
 ...
 $result=pjs_mysql_query($i_query);
}
