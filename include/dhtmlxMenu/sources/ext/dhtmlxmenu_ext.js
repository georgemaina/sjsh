...
dhtmlXMenuObject.prototype.setItemText = function(id, text) {
	...
	if ((this.itemPull[id] != null) && (this.idPull[id] != null)) {
		...
		if (this.itemPull[id]["parent"] == this.idPrefix+this.topId) { this._redistribTopLevelPositions(); }
	}
}
/****************************************************************************************************************************************************/
/**
...
dhtmlXMenuObject.prototype._updateLoaderIcon = function(id, state) {
	...
	for (var q=0; q<this.idPull[id].childNodes.length; q++) {
		if (String(this.idPull[id].childNodes[q].tagName).toLowerCase() == "div") {
			if (this.idPull[id].childNodes[q].className == "dhtmlxMenu_SubLevelArea_Item_Arrow_Loading") {
				// set defaule arrow
				var arw = "arrow"+(this._rtl?"r":"l")+"_"+(this.itemPull[id]["state"]=="enabled"?"en":"dis");
				this.idPull[id].childNodes[q].className = "dhtmlxMenu_SubLevelArea_Item_Arrow "+arw;
			} else {
				// set loader icon
				this.idPull[id].childNodes[q].className = "dhtmlxMenu_SubLevelArea_Item_Arrow_Loading";
			}
		}
	}
}
...
dhtmlXMenuObject.prototype.setHotKey = function(id, hkey) {
	...
	// var spnObj = null;
	for (var q=0; q<this.idPull[id].childNodes.length; q++) {
		if (this.idPull[id].childNodes[q].tagName == "DIV" && this.idPull[id].childNodes[q].className == "dhtmlxMenu_SubLevelArea_Item_HotKey") { hkObj = this.idPull[id].childNodes[q]; }
	...
	if (hkey.length == 0) {
		...
		if (hkObj == null) {
			...
			hkObj.className = "dhtmlxMenu_SubLevelArea_Item_HotKey";
...
dhtmlXMenuObject.prototype.setRTL = function(state) {
	...
	for (var a in this.itemPull) {
		...
		if (this.itemPull[a]["parent"] == this.idPrefix+this.topId) {
			if ((this.itemPull[a]["state"]=="enabled"?this.itemPull[a]["imgen"]:this.itemPull[a]["imgdis"]) != "") {
				this.idPull[a].style.paddingLeft = (this._rtl?this.topLevelItemPaddingIconNotExists:this.topLevelItemPaddingIconExists);
				this.idPull[a].style.paddingRight = (this._rtl?this.topLevelItemPaddingIconExists:this.topLevelItemPaddingIconNotExists);
				for (var q=0; q<this.idPull[a].childNodes.length; q++) { if (String(this.idPull[a].childNodes[q].tagName).toLowerCase() == "img") { this.idPull[a].childNodes[q].className = "dhtmlxMenu_TopLevel_Item_Icon_"+(this._rtl?"right":"left"); } }
			}
		}
...
dhtmlXMenuObject.prototype._setRadioState = function(id, state) {
	...
	if (imgObj != null) {
		...
		rObj["imgdis"] = "rdbtdis_"+(rObj["checked"]?"1":"0");
		imgObj.className = "dhtmlxMenu_SubLevelArea_Item_Icon "+(rObj["state"]=="disabled"?rObj["imgdis"]:rObj["imgen"]);
	}
}
...
dhtmlXMenuObject.prototype._updateCheckboxImage = function(id) {
	...
	this.itemPull[this.idPrefix+id]["imgdis"] = "chbxdis_"+(this._getCheckboxState(id)?"1":"0");
	// search image, not childNodes[0]
	this.idPull[this.idPrefix+id].childNodes[0].className = "dhtmlxMenu_SubLevelArea_Item_Icon "+(this.itemPull[this.idPrefix+id]["state"]=="enabled"?this.itemPull[this.idPrefix+id]["imgen"]:this.itemPull[this.idPrefix+id]["imgdis"]);
}
