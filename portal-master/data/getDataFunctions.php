...
function getVitals($encNo)
{
    ...
    while ($row = $result->FetchRow()) {
        ...
        echo '{"EncounterNo":"' . $row[encounter_nr] . '","VitalsTime":"' . $row[create_time]
            ...
            . '","Value":"' . $row[value] .'","Lower":' . $row[lower].',"Upper":' . $row[upper]. '}';
...
function getPrescriptions($encNo){
    ...
    $sql="SELECT d.`status`,p.`pid`,d.`encounter_nr`,d.`nr`,d.`drug_class`,d.partcode,d.`article`,d.`dosage`,d.`days`,d.`times_per_day`,d.`price`,
            ...
            WHERE d.`drug_class`='Drug_list' and d.encounter_nr='$encNo'";
    if($debug) echo $sql;
...
function getOpdPatients(){
    ...
    $sql = "SELECT d.`status`,c.`pid`,CONCAT(c.`name_first`,' ',c.`name_2`,' ', c.`name_last`) AS pnames, IF(t.`name`<>'',t.`name`,'CASH PAYMENT') AS PaymentMode,c.`date_birth`, c.`sex`,e.`name_formal`,
              d.`encounter_date`,d.encounter_nr,d.encounter_time
            FROM care_person c
    ...
    while ($row = $result->FetchRow()) {
        ...
        echo '{"Status":"'. $status.'","Pid":"'. $row[pid].'","Names":"'. $row[pnames].'","PaymentMode":"'. $payMode.'","EncounterTime":"'. $row[encounter_time]
            .'","Gender":"'. $gender .'","Clinic":"'. $row[name_formal].'","EncounterNo":"'. $row[encounter_nr].'","DOB":"'. $row[date_birth].'","Age":"'. trim($age).'"}';
        if ($counter<>$numRows){
            echo ",";
        }
        $counter++;
